...
-(void) handleCMD:(NSString*)cmd param:(NSDictionary*)param {
    ...
    }else if([cmd isEqualToString:WKCMDSyncPinnedMessage]) { // 同步置顶消息
        
        WKChannel *channel = [[WKChannel alloc] initWith:param[@"channel_id"] channelType:[param[@"channel_type"] intValue]];
        [[NSNotificationCenter defaultCenter] postNotificationName:WKNOTIFY_SYNC_PINNED_MESSAGE object:channel];
