...
@interface WKConversationView ()<WKConversationInputPanelDelegate,WKMultiplePanelDelegate,WKPinnedMessageManagerDelegate>
...
@property(nonatomic,assign) BOOL pinnedSyncing;
...
-(void) viewDidLoad {
    ...
    [self syncPinnedMessages];
    [self refreshPinnedTopViewAnimated:NO];
...
-(void) addDelegates {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(syncPinnedMessageNotification:) name:WKNOTIFY_SYNC_PINNED_MESSAGE object:nil];
...
-(void) removeDelegates {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:WKNOTIFY_SYNC_PINNED_MESSAGE object:nil];
...
-(void) syncPinnedMessageNotification:(NSNotification*)notification {
    WKChannel *channel = notification.object;
    if(channel && ![channel isEqual:self.channel]) {
        return;
    }
    [self syncPinnedMessages];
}
...
-(void) pinnedMessageChange:(WKChannel *)channel {
    ...
    [self refreshPinnedTopViewAnimated:YES];
}

-(void) syncPinnedMessages {
    if(self.pinnedSyncing) {
        return;
    }
    self.pinnedSyncing = YES;
    uint64_t version = [[WKSDK shared].pinnedMessageManager getMaxVersion:self.channel];
    ...
    [[WKAPIClient sharedClient] POST:@"message/pinned/sync" parameters:@{
        ...
        weakSelf.pinnedSyncing = NO;
        NSArray<NSDictionary*> *messageDicts = result[@"messages"];
        ...
        NSArray<NSDictionary*> *pinnedDicts = result[@"pinned_messages"];
        if(pinnedDicts && pinnedDicts.count>0) {
            NSMutableArray<WKPinnedMessage*> *pinnedMessages = [NSMutableArray array];
            for (NSDictionary *pinnedDict in pinnedDicts) {
                WKPinnedMessage *pinnedMessage = [weakSelf toPinnedMessage:pinnedDict];
                if(pinnedMessage.messageId > 0) {
                    [pinnedMessages addObject:pinnedMessage];
                }
            }
            [[WKSDK shared].pinnedMessageManager addOrUpdatePinnedMessages:pinnedMessages];
        }else {
            [weakSelf refreshPinnedTopViewAnimated:YES];
        }
    }).catch(^(NSError *error){
        weakSelf.pinnedSyncing = NO;
        WKLogError(@"同步置顶消息失败！-> %@", error);
        [weakSelf refreshPinnedTopViewAnimated:YES];
    });
}

-(WKPinnedMessage*) toPinnedMessage:(NSDictionary*)dict {
    WKPinnedMessage *pinnedMessage = [WKPinnedMessage new];
    pinnedMessage.messageId = [self unsignedLongLongFromValue:dict[@"message_id"]];
    pinnedMessage.messageSeq = (uint32_t)[self unsignedLongLongFromValue:dict[@"message_seq"]];
    NSString *channelID = dict[@"channel_id"] ?: self.channel.channelId ?: @"";
    NSInteger channelType = dict[@"channel_type"] ? [dict[@"channel_type"] integerValue] : self.channel.channelType;
    pinnedMessage.channel = [WKChannel channelID:channelID channelType:channelType];
    pinnedMessage.isDeleted = dict[@"is_deleted"] ? [dict[@"is_deleted"] boolValue] : false;
    pinnedMessage.version = [self unsignedLongLongFromValue:dict[@"version"]];
    return pinnedMessage;
}

-(uint64_t) unsignedLongLongFromValue:(id)value {
    if(!value || value == [NSNull null]) {
        return 0;
    }
    if([value isKindOfClass:NSString.class]) {
        return [[[NSDecimalNumber alloc] initWithString:value] unsignedLongLongValue];
    }
    return [value unsignedLongLongValue];
}

-(void) refreshPinnedTopViewAnimated:(BOOL)animated {
    NSArray<WKMessage*> *messages = [[WKSDK shared].pinnedMessageManager getPinnedMessagesByChannel:self.channel];
    WKMessage *message = messages.lastObject;
    if(!message) {
        [self showTopView:NO animated:animated];
        return;
    }
    [self.topView refreshWithPinnedMessage:message];
    [self showTopView:YES animated:animated];
}

-(void) togglePinnedMessage:(WKMessage*)message {
    if(message.channel.channelType == WK_GROUP && ![[WKSDK shared].channelManager isManager:message.channel memberUID:[WKApp shared].loginInfo.uid]) {
        [[WKNavigationManager shared].topViewController.view showMsg:LLang(@"只有群主或管理员可以操作")];
        return;
    }
    __weak typeof(self) weakSelf = self;
    [[WKAPIClient sharedClient] POST:@"message/pinned" parameters:@{
        ...
        [weakSelf syncPinnedMessages];
    }).catch(^(NSError *error){
        [[WKNavigationManager shared].topViewController.view showMsg:error.domain ?: LLang(@"置顶失败")];
    });
}
...
- (WKConversationTopView *)topView {
    if(!_topView) {
        __weak typeof(self) weakSelf = self;
        _topView = [[WKConversationTopView alloc] initWithFrame:CGRectMake(0.0, 0.0f, self.lim_width, 54.0f)];
        _topView.hidden = YES;
        _topView.lim_top = -_topView.lim_height;
        _topView.onTap = ^(WKMessage * _Nonnull message) {
            [weakSelf.messageListView locateMessageCell:message.messageSeq];
        };
        _topView.onClose = ^(WKMessage * _Nonnull message) {
            [weakSelf togglePinnedMessage:message];
        };
    }
    return _topView;
}
