...
static BOOL WKMessageHasVisibleReactions(WKMessageModel *model) {
    if(!model.reactions || model.reactions.count == 0) {
        return false;
    }
    for (WKReaction *reaction in model.reactions) {
        if(!reaction.isDeleted && reaction.emoji && ![reaction.emoji isEqualToString:@""]) {
            return true;
        }
    }
    return false;
...
static UIColor *WKMessageSentBubbleColor(void) {
    return [UIColor colorWithRed:47.0f/255.0f green:112.0f/255.0f blue:245.0f/255.0f alpha:1.0f];
}
...
- (void)refresh:(WKMessageModel *)model {
    ...
    // 回应
    if(WKMessageHasVisibleReactions(self.messageModel)) {
        self.reactionView.hidden = NO;
        [self.reactionView render:self.messageModel.reactions];
    ...
    if(self.messageModel.isSend) {
        self.bubbleBackgroundView.tintColor = WKMessageSentBubbleColor();
    }else{
...
-(void) layoutReaction {
    if(self.reactionView.hidden || self.reactionView.lim_width <= 0.0f || self.reactionView.lim_height <= 0.0f) {
        return;
    }
    CGFloat horizontalInset = 12.0f;
    if([self.messageModel isSend]) {
        self.reactionView.lim_left = self.bubbleBackgroundView.lim_right - self.reactionView.lim_width - horizontalInset;
    }else{
        self.reactionView.lim_left = self.bubbleBackgroundView.lim_left + horizontalInset;
    }
    
    self.reactionView.lim_top = self.bubbleBackgroundView.lim_bottom + 2.0f;
}
...
+(UIEdgeInsets) bubbleEdgeInsets:(WKMessageModel*) model contentSize:(CGSize)contentSize{
    ...
    if(WKMessageHasVisibleReactions(model)) {
        bottom += 34.0f;
    }
