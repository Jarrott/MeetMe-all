      ...
      <div class="flex-1 overflow-hidden p-12px">
        <div class="record-chat-panel">
          <div class="record-chat-toolbar">
            <div>
              <div class="record-chat-title">{{ $route.query.name }} / {{ $route.query.toname }}</div>
              <div class="record-chat-subtitle">单聊消息记录 · 共 {{ total }} 条</div>
            </div>
            <el-tag type="primary" effect="plain">单聊</el-tag>
          </div>
...
const senderName = (item: any) => {
  const uid = queryText(route.query.uid);
  const touid = queryText(route.query.touid);
  if (item?.sender === uid) {
    return queryText(route.query.name) || item?.sender_name || item?.sender || '未知用户';
  }
  if (item?.sender === touid) {
    return queryText(route.query.toname) || item?.sender_name || item?.sender || '未知用户';
  }
...
const isOutgoing = (item: any) => {
  const uid = queryText(route.query.uid);
  return Boolean(uid && item?.sender === uid);
};
...
const getUserList = async () => {
  ...
  messageRecordpersonalGet(queryFrom)
    .then((res: any) => {
...
const onDevices = (item?: any) => {
  const uid = item?.sender || queryText(route.query.uid);
  if (uid) {
...
.record-chat-item.is-outgoing .record-meta {
