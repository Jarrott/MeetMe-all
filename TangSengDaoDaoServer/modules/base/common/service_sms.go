...
func (s *SMSService) SendVerifyCode(ctx context.Context, zone, phone string, codeType CodeType) error {
	...
	smsProviderName := strings.ToLower(strings.TrimSpace(os.Getenv("SMS_PROVIDER")))
	if smsProviderName == "" {
		smsProviderName = strings.ToLower(strings.TrimSpace(string(s.ctx.GetConfig().SMSProvider)))
	}
	switch smsProviderName {
	case "3yit", "yit", "yit_sms":
		smsProvider = NewYitSMSProvider(s.ctx)
	case "textboost":
		smsProvider = NewTextboostSMSProvider(s.ctx)
	case "auto":
		if isYitSMSConfigured() {
			smsProvider = NewYitSMSProvider(s.ctx)
		} else {
			smsProvider = NewTextboostSMSProvider(s.ctx)
		}
	case string(config.SMSProviderAliyun):
	...
	case string(config.SMSProviderUnisms):
		smsProvider = NewUnismsProvider(s.ctx)
	case string(config.SMSProviderSmsbao):
