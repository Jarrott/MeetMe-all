...
func (cn *Common) Route(r *wkhttp.WKHttp) {
	...
	sticker := r.Group("/v1/sticker", cn.ctx.AuthMiddleware(r))
	{
		sticker.GET("/user/category", cn.userStickerCategory)
		sticker.GET("/user/sticker", cn.userStickerList)
		sticker.POST("/user/sticker", cn.userStickerAdd)
		sticker.DELETE("/user/sticker/:id", cn.userStickerDelete)
	}
...
func (cn *Common) userStickerCategory(c *wkhttp.Context) {
	uid := c.GetLoginUID()
	model, err := cn.db.queryFirstUserSticker(uid)
	if err != nil {
		cn.Error("查询用户贴图分类错误", zap.Error(err))
		c.ResponseError(errors.New("查询用户贴图分类错误"))
		return
	}
	cover := ""
	if model != nil {
...
func (cn *Common) userStickerList(c *wkhttp.Context) {
	uid := c.GetLoginUID()
	category := strings.TrimSpace(c.Query("category"))
	if category == "" {
		category = "custom"
	}
	list, err := cn.db.queryUserStickers(uid, category)
	if err != nil {
		cn.Error("查询用户贴图错误", zap.Error(err))
		c.ResponseError(errors.New("查询用户贴图错误"))
		return
	}
	resps := make([]*userStickerResp, 0, len(list))
	for _, item := range list {
		resps = append(resps, &userStickerResp{
			ID:          item.Id,
	...
	c.Response(gin.H{"list": resps})
}

func (cn *Common) userStickerAdd(c *wkhttp.Context) {
	var req userStickerAddReq
	...
	if req.Path == "" {
		c.ResponseError(errors.New("贴图地址不能为空"))
		return
	}
	if req.Category == "" {
		req.Category = "custom"
	...
	if m.Placeholder == "" {
		m.Placeholder = m.Path
	}
	id, err := cn.db.insertUserSticker(m)
	if err != nil {
		cn.Error("添加用户贴图错误", zap.Error(err))
		c.ResponseError(errors.New("添加用户贴图错误"))
		return
	}
	m.Id = id
	c.Response(&userStickerResp{
		ID:          m.Id,
		Category:    m.Category,
		Path:        m.Path,
...
func (cn *Common) userStickerDelete(c *wkhttp.Context) {
	id, _ := strconv.ParseInt(c.Param("id"), 10, 64)
	if id <= 0 {
		c.ResponseError(errors.New("贴图ID不能为空"))
		return
	}
	if err := cn.db.deleteUserSticker(c.GetLoginUID(), id); err != nil {
		cn.Error("删除用户贴图错误", zap.Error(err))
		c.ResponseError(errors.New("删除用户贴图错误"))
		return
	}
	c.ResponseOK()
}
...
func (cn *Common) insertAppConfigIfNeed() (*appConfigModel, error) {
	...
	appConfigM = &appConfigModel{
		...
		SuperTokenOn:                  0,
		SearchByPhone:                 1,
		ConversationTeamEnabled:       1,
		GroupMemberPrivateChatEnabled: 1,
	}
...
func (cn *Common) appConfig(c *wkhttp.Context) {
	...
	c.JSON(http.StatusOK, &appConfigResp{
		...
		AutoTranslateOn:                appConfigM.AutoTranslateOn,
		ConversationTeamEnabled:        appConfigM.ConversationTeamEnabled,
		GroupMemberPrivateChatEnabled:  appConfigM.GroupMemberPrivateChatEnabled,
	})
}
