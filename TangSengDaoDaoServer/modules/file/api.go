package file

import (
	"crypto/sha512"
	"encoding/base64"
	"errors"
	"fmt"
	"io"
	"net/http"
	"net/url"
	"os"
	"path/filepath"
	"strconv"
	"strings"

	"github.com/TangSengDaoDao/TangSengDaoDaoServerLib/config"
	"github.com/TangSengDaoDao/TangSengDaoDaoServerLib/pkg/log"
	"github.com/TangSengDaoDao/TangSengDaoDaoServerLib/pkg/util"
	"github.com/TangSengDaoDao/TangSengDaoDaoServerLib/pkg/wkhttp"
	"github.com/gin-gonic/gin"
	"go.uber.org/zap"
)

// File 文件操作
type File struct {
	ctx *config.Context
	log.Log
	service IService
}

// New New
func New(ctx *config.Context) *File {
	return &File{
		ctx:     ctx,
		Log:     log.NewTLog("File"),
		service: NewService(ctx),
	}
}

// Route 路由
func (f *File) Route(r *wkhttp.WKHttp) {
	api := r.Group("/v1/file")
	{ // 文件上传
		// api.POST("/upload/*path", f.upload)
		// 组合图片
		//	api.POST("/compose/*path", f.makeImageCompose)
		// 获取文件
		api.GET("/preview/*path", f.getFile)
	}
	auth := r.Group("/v1/file", f.ctx.AuthMiddleware(r))
	{
		//获取上传文件地址
		auth.GET("/upload", f.getFilePath)
		//上传文件
		auth.POST("/upload", f.uploadFile)
		auth.POST("/chunk/init", f.initChunkUpload)
		auth.POST("/chunk/upload", f.uploadChunk)
		auth.POST("/chunk/complete", f.completeChunkUpload)
	}
}

type initChunkUploadReq struct {
	Type        string `json:"type"`
	Path        string `json:"path"`
	ContentType string `json:"contenttype"`
	Size        int64  `json:"size"`
	ChunkSize   int64  `json:"chunk_size"`
	TotalChunks int    `json:"total_chunks"`
}

type completeChunkUploadReq struct {
	UploadID    string `json:"upload_id"`
	Type        string `json:"type"`
	Path        string `json:"path"`
	ContentType string `json:"contenttype"`
	TotalChunks int    `json:"total_chunks"`
}

const maxChunkUploadCount = 10000

func (f *File) initChunkUpload(c *wkhttp.Context) {
	var req initChunkUploadReq
	if err := c.BindJSON(&req); err != nil {
		c.ResponseError(errors.New("数据格式有误"))
		return
	}
	err := f.checkReq(Type(req.Type), req.Path)
	if err != nil {
		c.ResponseError(err)
		return
	}
	if req.TotalChunks <= 0 || req.TotalChunks > maxChunkUploadCount {
		c.ResponseError(errors.New("分片数量错误"))
		return
	}
	uploadID := util.GenerUUID()
	dir := f.chunkUploadDir(c.GetLoginUID(), uploadID)
	if err := os.MkdirAll(dir, 0755); err != nil {
		f.Error("创建分片目录失败！", zap.Error(err))
		c.ResponseError(errors.New("创建分片上传失败"))
		return
	}
	c.Response(map[string]interface{}{
		"upload_id":    uploadID,
		"chunk_url":    f.normalizeURLForRequest(c, fmt.Sprintf("%s/file/chunk/upload", f.ctx.GetConfig().External.APIBaseURL)),
		"complete_url": f.normalizeURLForRequest(c, fmt.Sprintf("%s/file/chunk/complete", f.ctx.GetConfig().External.APIBaseURL)),
		"path":         req.Path,
	})
}

func (f *File) uploadChunk(c *wkhttp.Context) {
	uploadID := c.PostForm("upload_id")
	indexText := c.PostForm("index")
	if !isSafeUploadID(uploadID) {
		c.ResponseError(errors.New("上传ID错误"))
		return
	}
	index, err := strconv.Atoi(indexText)
	if err != nil || index < 0 || index >= maxChunkUploadCount {
		c.ResponseError(errors.New("分片序号错误"))
		return
	}
	file, _, err := c.Request.FormFile("file")
	if err != nil {
		f.Error("读取分片失败！", zap.Error(err))
		c.ResponseError(errors.New("读取分片失败"))
		return
	}
	defer file.Close()
	dir := f.chunkUploadDir(c.GetLoginUID(), uploadID)
	if err := os.MkdirAll(dir, 0755); err != nil {
		f.Error("创建分片目录失败！", zap.Error(err))
		c.ResponseError(errors.New("创建分片目录失败"))
		return
	}
	dst, err := os.Create(filepath.Join(dir, fmt.Sprintf("%06d.part", index)))
	if err != nil {
		f.Error("创建分片文件失败！", zap.Error(err))
		c.ResponseError(errors.New("创建分片文件失败"))
		return
	}
	defer dst.Close()
	if _, err = io.Copy(dst, file); err != nil {
		f.Error("保存分片失败！", zap.Error(err))
		c.ResponseError(errors.New("保存分片失败"))
		return
	}
	c.Response(map[string]interface{}{
		"upload_id": uploadID,
		"index":     index,
	})
}

func (f *File) completeChunkUpload(c *wkhttp.Context) {
	var req completeChunkUploadReq
	if err := c.BindJSON(&req); err != nil {
		c.ResponseError(errors.New("数据格式有误"))
		return
	}
	if !isSafeUploadID(req.UploadID) {
		c.ResponseError(errors.New("上传ID错误"))
		return
	}
	if req.TotalChunks <= 0 || req.TotalChunks > maxChunkUploadCount {
		c.ResponseError(errors.New("分片数量错误"))
		return
	}
	err := f.checkReq(Type(req.Type), req.Path)
	if err != nil {
		c.ResponseError(err)
		return
	}
	path := req.Path
	if !strings.HasPrefix(path, "/") {
		path = fmt.Sprintf("/%s", path)
	}
	dir := f.chunkUploadDir(c.GetLoginUID(), req.UploadID)
	defer os.RemoveAll(dir)
	contentType := req.ContentType
	if contentType == "" {
		contentType = "application/octet-stream"
	}
	_, err = f.service.UploadFile(fmt.Sprintf("%s%s", req.Type, path), contentType, func(w io.Writer) error {
		for i := 0; i < req.TotalChunks; i++ {
			partPath := filepath.Join(dir, fmt.Sprintf("%06d.part", i))
			part, err := os.Open(partPath)
			if err != nil {
				return err
			}
			_, copyErr := io.Copy(w, part)
			closeErr := part.Close()
			if copyErr != nil {
				return copyErr
			}
			if closeErr != nil {
				return closeErr
			}
		}
		return nil
	})
	if err != nil {
		f.Error("合并上传文件失败！", zap.Error(err))
		c.ResponseError(errors.New("合并上传文件失败"))
		return
	}
	c.Response(map[string]string{
		"path": fmt.Sprintf("file/preview/%s%s", req.Type, path),
	})
}

func (f *File) chunkUploadDir(uid string, uploadID string) string {
	return filepath.Join(os.TempDir(), "meetme-upload-chunks", uid, uploadID)
}

func isSafeUploadID(uploadID string) bool {
	if len(uploadID) < 16 || len(uploadID) > 80 {
		return false
	}
	for _, r := range uploadID {
		if (r >= 'a' && r <= 'z') || (r >= 'A' && r <= 'Z') || (r >= '0' && r <= '9') || r == '-' || r == '_' {
			continue
		}
		return false
	}
	return true
}

func (f *File) makeImageCompose(c *wkhttp.Context) {
	var imageURLs []string
	if err := c.BindJSON(&imageURLs); err != nil {
		f.Error("数据格式有误！", zap.Error(err))
		c.ResponseError(errors.New("数据格式有误！"))
		return
	}
	if len(imageURLs) <= 0 {
		c.ResponseError(errors.New("图片不能为空！"))
		return
	}
	if len(imageURLs) > 9 {
		c.ResponseError(errors.New("图片数量不能大于9！"))
		return
	}
	uploadPath := c.Param("path")
	// 下载并组合图片
	resultMap, err := f.service.DownloadAndMakeCompose(uploadPath, imageURLs)
	if err != nil {
		f.Error("组合图片失败！", zap.String("uploadPath", uploadPath), zap.Any("imageURLs", imageURLs), zap.Error(err))
		c.ResponseError(errors.New("组合图片失败！"))
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"path": resultMap["fid"].(string),
	})
}

// 获取上传文件地址
func (f *File) getFilePath(c *wkhttp.Context) {
	loginUID := c.GetLoginUID()
	uploadPath := c.Query("path")
	fileType := c.Query("type")
	err := f.checkReq(Type(fileType), uploadPath)
	if err != nil {
		c.ResponseError(err)
		return
	}
	var path string
	if Type(fileType) == TypeMomentCover {
		// 动态封面
		path = fmt.Sprintf("%s/file/upload?type=%s&path=/%s.png", f.ctx.GetConfig().External.APIBaseURL, fileType, loginUID)
	} else if Type(fileType) == TypeSticker {
		// 自定义表情
		ext := safeStickerExt(c.Query("ext"))
		path = fmt.Sprintf("%s/file/upload?type=%s&path=/%s/%s%s", f.ctx.GetConfig().External.APIBaseURL, fileType, loginUID, util.GenerUUID(), ext)
	} else if Type(fileType) == TypeWorkplaceBanner {
		// 工作台横幅
		path = fmt.Sprintf("%s/file/upload?type=%s&path=/workplace/banner/%s", f.ctx.GetConfig().External.APIBaseURL, fileType, path)
	} else if Type(fileType) == TypeWorkplaceAppIcon {
		// 工作台appIcon
		path = fmt.Sprintf("%s/file/upload?type=%s&path=/workplace/appicon/%s", f.ctx.GetConfig().External.APIBaseURL, fileType, path)
	} else {
		path = fmt.Sprintf("%s/file/upload?type=%s&path=%s", f.ctx.GetConfig().External.APIBaseURL, fileType, uploadPath)
	}
	c.Response(map[string]string{
		"url": f.normalizeURLForRequest(c, path),
	})
}

func safeStickerExt(ext string) string {
	ext = strings.ToLower(strings.TrimSpace(ext))
	if ext == "" {
		return ".gif"
	}
	if !strings.HasPrefix(ext, ".") {
		ext = "." + ext
	}
	switch ext {
	case ".gif", ".png", ".jpg", ".jpeg", ".webp", ".svg":
		return ext
	default:
		return ".gif"
	}
}

// 上传文件
func (f *File) uploadFile(c *wkhttp.Context) {
	uploadPath := c.Query("path")
	fileType := c.Query("type")
	signature := c.Query("signature") // 是否返回签名
	var signatureInt int64 = 0
	if signature != "" {
		signatureInt, _ = strconv.ParseInt(signature, 10, 64)
	}
	contentType := c.DefaultPostForm("contenttype", "application/octet-stream")
	err := f.checkReq(Type(fileType), uploadPath)
	if err != nil {
		c.ResponseError(err)
		return
	}
	file, _, err := c.Request.FormFile("file")
	if err != nil {
		f.Error("读取文件失败！", zap.Error(err))
		c.ResponseError(errors.New("读取文件失败！"))
		return
	}
	path := uploadPath
	if !strings.HasPrefix(path, "/") {
		path = fmt.Sprintf("/%s", path)
	}
	var sign []byte
	if signatureInt == 1 {
		// bytes, err := ioutil.ReadAll(file)
		// if err != nil {
		// 	f.Error("读取文件错误", zap.Error(err))
		// 	c.ResponseError(errors.New("读取文件错误"))
		// 	return
		// }
		h := sha512.New()
		_, err := io.Copy(h, file)
		if err != nil {
			f.Error("签名复制文件错误", zap.Error(err))
			c.ResponseError(errors.New("签名复制文件错误"))
			return
		}
		sign = h.Sum(nil)
		//	sign = sha512.Sum512(bytes)

	}
	_, err = f.service.UploadFile(fmt.Sprintf("%s%s", fileType, path), contentType, func(w io.Writer) error {
		_, err := file.Seek(0, io.SeekStart)
		if err != nil {
			f.Error("设置文件偏移量错误", zap.Error(err))
			return err
		}
		_, err = io.Copy(w, file)
		return err
	})

	defer file.Close()
	if err != nil {
		f.Error("上传文件失败！", zap.Error(err))
		c.ResponseError(errors.New("上传文件失败！"))
		return
	}
	if signatureInt == 1 {
		encoded := base64.StdEncoding.EncodeToString(sign[:])
		fmt.Print("编码文件", encoded)
		c.Response(map[string]interface{}{
			"path":   fmt.Sprintf("file/preview/%s%s", fileType, path),
			"sha512": encoded,
		})
	} else {
		c.Response(map[string]string{
			"path": fmt.Sprintf("file/preview/%s%s", fileType, path),
		})
	}
}

// 获取文件
func (f *File) getFile(c *wkhttp.Context) {
	ph := c.Param("path")
	if ph == "" {
		c.Response(errors.New("访问路径不能为空"))
		return
	}
	filename := c.Query("filename")
	if filename == "" {
		paths := strings.Split(ph, "/")
		if len(paths) > 0 {
			filename = paths[len(paths)-1]
		}
	}
	width, height := parsePreviewSize(c.Query("width"), c.Query("height"))
	isDownload := c.Query("download") == "1"
	var downloadURL string
	var err error
	if !isDownload && width > 0 && height > 0 {
		downloadURL, err = f.service.ImagePreviewURL(ph, filename, width, height)
	} else {
		downloadURL, err = f.service.DownloadURL(ph, filename)
	}
	if err != nil {
		c.ResponseError(err)
		return
	}
	if isDownload {
		downloadURL = forceDownloadURL(downloadURL, filename)
	}
	downloadURL = f.normalizeURLForRequest(c, downloadURL)
	c.Redirect(http.StatusFound, downloadURL)
}

func parsePreviewSize(widthText string, heightText string) (int, int) {
	width, _ := strconv.Atoi(widthText)
	height, _ := strconv.Atoi(heightText)
	if width <= 0 || height <= 0 {
		return 0, 0
	}
	return width, height
}

func forceDownloadURL(rawURL string, filename string) string {
	if rawURL == "" {
		return rawURL
	}
	parsed, err := url.Parse(rawURL)
	if err != nil {
		return rawURL
	}
	vals := parsed.Query()
	vals.Set("response-content-disposition", fmt.Sprintf("attachment; filename=\"%s\"", filename))
	parsed.RawQuery = vals.Encode()
	return parsed.String()
}

func (f *File) normalizeURLForRequest(c *wkhttp.Context, rawURL string) string {
	if rawURL == "" {
		return rawURL
	}
	proto := c.Request.Header.Get("X-Forwarded-Proto")
	if proto == "" {
		proto = c.Request.Header.Get("X-Forwarded-Scheme")
	}
	if proto == "" && strings.EqualFold(c.Request.Header.Get("X-Forwarded-Ssl"), "on") {
		proto = "https"
	}
	if proto == "" && strings.Contains(c.Request.Header.Get("CF-Visitor"), `"scheme":"https"`) {
		proto = "https"
	}
	if proto == "" && c.Request.TLS != nil {
		proto = "https"
	}
	if proto != "https" || !strings.HasPrefix(rawURL, "http://") {
		return rawURL
	}
	parsed, err := url.Parse(rawURL)
	if err != nil || parsed.Host == "" {
		return rawURL
	}
	parsed.Scheme = "https"
	return parsed.String()
}

func (f *File) checkReq(fileType Type, path string) error {
	if fileType == "" {
		return errors.New("文件类型不能为空")
	}
	if path == "" && fileType != TypeMomentCover && fileType != TypeSticker {
		return errors.New("上传路径不能为空")
	}
	if fileType != TypeChat && fileType != TypeMoment && fileType != TypeMomentCover && fileType != TypeSticker && fileType != TypeReport && fileType != TypeChatBg && fileType != TypeCommon && fileType != TypeDownload {
		return errors.New("文件类型错误")
	}
	return nil
}
