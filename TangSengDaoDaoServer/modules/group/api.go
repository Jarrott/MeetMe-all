...
func (g *Group) groupCreate(c *wkhttp.Context) {
	...
	groupName = trimGroupName(groupName)

	g.createGroupMux.Lock()
	defer g.createGroupMux.Unlock()

	duplicateGroup, missingMembers, err := g.findRecentDuplicateGroup(creator, groupName, realUids)
	if err != nil {
		g.Error("查询重复群失败！", zap.Error(err), zap.Strings("members", realUids))
		c.ResponseError(errors.New("查询重复群失败！"))
		return
	}
	if duplicateGroup != nil {
		if len(missingMembers) > 0 {
			if err := g.addMembers(missingMembers, duplicateGroup.GroupNo, creator, creatorName); err != nil {
				g.Error("补充重复群成员失败！", zap.Error(err), zap.String("groupNo", duplicateGroup.GroupNo), zap.Strings("members", missingMembers))
				c.ResponseError(errors.New("补充重复群成员失败！"))
				return
			}
			groupModel, err := g.db.QueryWithGroupNo(duplicateGroup.GroupNo)
			if err != nil {
				g.Error("查询群信息失败！", zap.Error(err))
				c.ResponseError(errors.New("查询群信息失败！"))
				return
			}
			duplicateGroup = groupModel
		}
		g.Info("命中重复建群保护，返回已有群", zap.String("groupNo", duplicateGroup.GroupNo), zap.String("creator", creator), zap.Strings("members", realUids))
		g.responseCreatedGroup(c, duplicateGroup)
		return
	}
	...
	g.responseCreatedGroup(c, groupModel)
}

const (
	groupCreateDuplicateWindow = 10 * time.Minute
	groupNameMaxRunes          = 50
)

func (g *Group) findRecentDuplicateGroup(creator string, groupName string, memberUids []string) (*Model, []string, error) {
	groupModels, err := g.db.queryRecentNormalGroupsWithCreator(creator, time.Now().Add(-groupCreateDuplicateWindow))
	if err != nil || len(groupModels) == 0 {
		return nil, nil, err
	}
	groupNos := make([]string, 0, len(groupModels))
	for _, groupModel := range groupModels {
		groupNos = append(groupNos, groupModel.GroupNo)
	}
	memberModels, err := g.db.queryMembersWithGroupNos(groupNos)
	if err != nil {
		return nil, nil, err
	}
	membersByGroup := make(map[string][]string, len(groupModels))
	for _, memberModel := range memberModels {
		membersByGroup[memberModel.GroupNo] = append(membersByGroup[memberModel.GroupNo], memberModel.UID)
	}
	targetKey := groupMemberSetKey(memberUids)
	for _, groupModel := range groupModels {
		existingMembers := membersByGroup[groupModel.GroupNo]
		if groupMemberSetKey(existingMembers) == targetKey {
			return groupModel, nil, nil
		}
		if normalizeGroupName(groupModel.Name) == normalizeGroupName(groupName) && isNearDuplicateMemberSet(existingMembers, memberUids) {
			return groupModel, missingMemberUids(existingMembers, memberUids), nil
		}
	}
	return nil, nil, nil
}

func groupMemberSetKey(uids []string) string {
	items := append([]string(nil), uids...)
	sort.Strings(items)
...
func trimGroupName(name string) string {
	nameRuns := []rune(name)
	if len(nameRuns) > groupNameMaxRunes {
		return string(nameRuns[:groupNameMaxRunes])
	}
	return name
}
...
func isNearDuplicateMemberSet(existingMembers []string, memberUids []string) bool {
	if len(existingMembers) == 0 || len(memberUids) == 0 {
		return false
	}
	intersection := memberIntersectionCount(existingMembers, memberUids)
	minCount := len(existingMembers)
	if len(memberUids) < minCount {
		minCount = len(memberUids)
	}
	return float64(intersection)/float64(minCount) >= 0.9
}

func memberIntersectionCount(a []string, b []string) int {
	memberMap := make(map[string]struct{}, len(a))
	for _, uid := range a {
		memberMap[uid] = struct{}{}
	}
	count := 0
	for _, uid := range b {
...
func missingMemberUids(existingMembers []string, memberUids []string) []string {
	existingMap := make(map[string]struct{}, len(existingMembers))
	for _, uid := range existingMembers {
		existingMap[uid] = struct{}{}
	}
	missing := make([]string, 0)
	for _, uid := range memberUids {
		if _, ok := existingMap[uid]; !ok {
...
func (g *Group) responseCreatedGroup(c *wkhttp.Context, groupModel *Model) {
	groupResp := &GroupResp{}
...
func (g *Group) groupUpdate(c *wkhttp.Context) {
	...
	for key, value := range groupMap {
		...
		case common.GroupAttrKeyName:
			group.Name = trimGroupName(value)
			groupMap[key] = group.Name
