...
func (m *Manager) Route(r *wkhttp.WKHttp) {
	auth := r.Group("/v1/manager", m.ctx.AuthMiddleware(r))
	{
		...
		auth.GET("/group/activity_rank", m.activityRank)             // 群活跃排行
		...
		auth.POST("/groups/:group_no/managers", m.addManagers)       // 设置群管理员
		auth.DELETE("/groups/:group_no/managers", m.removeManagers)  // 取消群管理员
	}
}

// 群活跃排行
func (m *Manager) activityRank(c *wkhttp.Context) {
	...
	list, err := m.managerDB.queryGroupActivityRank(keyword, uint64(pageSize), uint64(pageIndex), m.ctx.GetConfig().TablePartitionConfig.MessageTableCount)
	if err != nil {
		m.Error("查询群活跃排行错误", zap.Error(err))
		c.ResponseError(errors.New("查询群活跃排行错误"))
		return
	}
	count, err := m.managerDB.queryGroupActivityRankCount(keyword)
	if err != nil {
		m.Error("查询群活跃排行数量错误", zap.Error(err))
		c.ResponseError(errors.New("查询群活跃排行数量错误"))
		return
	}
...
func (m *Manager) addManagers(c *wkhttp.Context) {
	m.updateManagers(c, true)
}

func (m *Manager) removeManagers(c *wkhttp.Context) {
	m.updateManagers(c, false)
}

func (m *Manager) updateManagers(c *wkhttp.Context, add bool) {
	...
	if len(memberUIDs) <= 0 {
		c.ResponseError(errors.New("请选择需要操作的成员！"))
		return
	}
	groupModel, err := m.db.QueryWithGroupNo(groupNo)
	if err != nil {
	...
	for _, uid := range memberUIDs {
		if uid == groupModel.Creator {
			c.ResponseError(errors.New("群主不能设置或取消管理员！"))
			return
		}
	}
	version := m.ctx.GenSeq(common.GroupMemberSeqKey)
	if add {
		err = m.db.UpdateMembersToManager(groupNo, memberUIDs, version)
	} else {
		err = m.db.UpdateManagersToMember(groupNo, memberUIDs, version)
	}
	if err != nil {
		m.Error("更新群管理员失败！", zap.String("group_no", groupNo), zap.Any("member_uids", memberUIDs), zap.Error(err))
		c.ResponseError(errors.New("更新群管理员失败！"))
		return
	}
	if groupModel.Forbidden == 1 {
		managerOrCreaterUIDs, err := m.db.QueryGroupManagerOrCreatorUIDS(groupNo)
		if err != nil {
			m.Error("查询群主和管理员失败！", zap.Error(err))
			c.ResponseError(errors.New("查询群主和管理员失败！"))
			return
		}
		err = m.ctx.IMWhitelistSet(config.ChannelWhitelistReq{
			ChannelReq: config.ChannelReq{
			...
			UIDs: managerOrCreaterUIDs,
		})
		if err != nil {
			m.Error("设置白名单失败！", zap.Error(err))
			c.ResponseError(errors.New("设置白名单失败！"))
			return
		}
	}
	if err := m.notifyGroupMemberUpdate(groupModel, memberUIDs); err != nil {
		m.Error("发送群成员更新命令失败！", zap.Error(err))
		c.ResponseError(errors.New("发送群成员更新命令失败！"))
		return
	}
	c.ResponseOK()
}

func (m *Manager) notifyGroupMemberUpdate(groupModel *Model, memberUIDs []string) error {
	if groupModel.GroupType == int(GroupTypeCommon) {
		return m.ctx.SendCMD(config.MsgCMDReq{
			ChannelID:   groupModel.GroupNo,
			ChannelType: common.ChannelTypeGroup.Uint8(),
			CMD:         common.CMDGroupMemberUpdate,
			Param: map[string]interface{}{
				"group_no": groupModel.GroupNo,
			},
		})
	}
	for _, uid := range memberUIDs {
