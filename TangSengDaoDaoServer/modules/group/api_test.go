...
func TestGroupCreateDuplicateRecentSameMembersReturnsExisting(t *testing.T) {
	s, ctx := testutil.NewTestServer()
	f := New(ctx)
	f.Route(s.GetRoute())
	...
	createGroup := func(members []string) map[string]interface{} {
		w := httptest.NewRecorder()
		req, err := http.NewRequest("POST", "/v1/group/create", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
			"name":    "重复群",
			"members": members,
	...
	first := createGroup([]string{"10009", "10010"})
	second := createGroup([]string{"10010", "10009"})

	assert.Equal(t, first["group_no"], second["group_no"])
}

func TestGroupCreateNearDuplicateSameNameAddsMissingMembers(t *testing.T) {
	s, ctx := testutil.NewTestServer()
	f := New(ctx)
	f.Route(s.GetRoute())
	...
	for _, item := range []user.Model{
		{UID: "10009", Name: "张九"},
		{UID: "10010", Name: "李十"},
	} {
		err = f.userDB.Insert(&item)
		assert.NoError(t, err)
	}
	...
	first := createGroup([]string{"10009"})
	second := createGroup([]string{"10009", "10010"})

	assert.Equal(t, first["group_no"], second["group_no"])
	memberCount, err := f.db.QueryMemberCount(first["group_no"].(string))
	assert.NoError(t, err)
