...
func (d *DB) existCommonGroupMember(uid string, toUID string) (bool, error) {
	var count int64
	_, err := d.session.Select("count(distinct a.group_no)").
		From("group_member a").
		Join("group_member b", "a.group_no=b.group_no").
		Where("a.uid=? and b.uid=? and a.is_deleted=0 and b.is_deleted=0", uid, toUID).
		Load(&count)
	return count > 0, err
}
...
func (d *DB) queryRecentNormalGroupsWithCreator(creator string, since time.Time) ([]*Model, error) {
	var models []*Model
	_, err := d.session.Select("*").From("`group`").Where("creator=? and status=? and created_at>=?", creator, GroupStatusNormal, since).OrderDir("created_at", false).Load(&models)
	return models, err
}

func (d *DB) queryMembersWithGroupNos(groupNos []string) ([]*MemberModel, error) {
	if len(groupNos) == 0 {
		return nil, nil
	}
	var models []*MemberModel
	_, err := d.session.Select("*").From("group_member").Where("group_no in ? and is_deleted=0", groupNos).Load(&models)
	return models, err
}
