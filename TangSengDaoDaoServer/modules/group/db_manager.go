...
func (m *managerDB) queryGroupActivityRank(keyword string, pageSize, page uint64, messageTableCount int) ([]*managerGroupModel, error) {
	var list []*managerGroupModel
	activitySQL := buildGroupActivitySQL(messageTableCount)
	whereSQL := ""
	args := []interface{}{}
	if strings.TrimSpace(keyword) != "" {
		whereSQL = " where g.name like ? or g.group_no like ?"
		args = append(args, "%"+keyword+"%", "%"+keyword+"%")
	}
	args = append(args, (page-1)*pageSize, pageSize)
	sql := fmt.Sprintf(`select g.*,ifnull(a.message_count,0) message_count,ifnull(a.last_message_at,'') last_message_at
from `+"`group`"+` g
left join (%s) a on a.channel_id=g.group_no
%s
order by ifnull(a.message_count,0) desc, a.last_message_at desc, g.created_at desc
limit ?,?`, activitySQL, whereSQL)
	_, err := m.session.SelectBySql(sql, args...).Load(&list)
	return list, err
}

func (m *managerDB) queryGroupActivityRankCount(keyword string) (int64, error) {
	var count int64
	builder := m.session.Select("count(*)").From("`group`")
	if strings.TrimSpace(keyword) != "" {
		builder = builder.Where("name like ? or group_no like ?", "%"+keyword+"%", "%"+keyword+"%")
	}
	_, err := builder.Load(&count)
	return count, err
}

func buildGroupActivitySQL(messageTableCount int) string {
	if messageTableCount <= 0 {
		messageTableCount = 1
	}
	parts := make([]string, 0, messageTableCount)
	for i := 0; i < messageTableCount; i++ {
		table := "message"
		if i > 0 {
			table = fmt.Sprintf("message%d", i)
		}
		parts = append(parts, fmt.Sprintf("select channel_id,count(*) message_count,date_format(max(created_at),'%%Y-%%m-%%d %%H:%%i:%%s') last_message_at from `%s` where channel_type=%d group by channel_id", table, common.ChannelTypeGroup.Uint8()))
	}
	return fmt.Sprintf("select channel_id,sum(message_count) message_count,max(last_message_at) last_message_at from (%s) activity group by channel_id", strings.Join(parts, " union all "))
}
