...
func (m *Message) sendMsg(c *wkhttp.Context) {
	...
	if req.ReceiveChannelType == common.ChannelTypePerson.Uint8() {
		...
		if !sendUserIsFriend || !recvUserIsFriend {
			canSendByGroup, err := m.canPrivateChatByCommonGroup(uid, req.ReceiveChannelID)
			if err != nil {
				c.ResponseError(err)
				return
			}
...
func (m *Message) canPrivateChatByCommonGroup(uid string, toUID string) (bool, error) {
	appConfig, err := m.commonService.GetAppConfig()
	if err != nil {
		m.Error("查询应用配置错误", zap.Error(err))
		return false, errors.New("查询应用配置错误")
	}
	if appConfig == nil || appConfig.GroupMemberPrivateChatEnabled != 1 {
		return false, nil
	}
	exist, err := m.groupService.ExistCommonGroupMember(uid, toUID)
	if err != nil {
		m.Error("查询共同群成员关系错误", zap.Error(err))
		return false, errors.New("查询共同群成员关系错误")
	}
	return exist, nil
}
...
func (m *Message) messageReaded(c *wkhttp.Context) {
	...
	if len(req.MessageIDs) == 0 && len(req.MessageSeqs) == 0 {
		c.ResponseError(errors.New("message_ids或message_seqs不能为空！"))
		return
	}
	// var cloneNo string
	fakeChannelID := req.ChannelID
	...
	if len(req.MessageSeqs) > 0 {
		...
		if syncMsg != nil {
			messages = syncMsg.Messages
		}
	} else {
		localMessages, err := m.db.queryMessagesByIDs(fakeChannelID, req.ChannelType, messageIdsI)
		if err != nil {
			c.ResponseErrorf("查询本地消息失败！", err)
			return
		}
		for _, localMessage := range localMessages {
			messages = append(messages, &config.MessageResp{
				MessageID:  localMessage.MessageID,
				MessageSeq: localMessage.MessageSeq,
				FromUID:    localMessage.FromUID,
			})
		}
	}
	if len(messages) <= 0 {
	...
	m.updateReadedCountAndNotify(fakeChannelID, req.ChannelID, req.ChannelType, loginUID, messages)
	// 异步处理 Redis 缓存
	go func() {
		for _, message := range messages {
...
func (m *Message) updateReadedCountAndNotify(fakeChannelID, reqChannelID string, reqChannelType uint8, loginUID string, messages []*config.MessageResp) {
	if len(messages) == 0 {
		return
	}
	messageIDStrs := make([]string, 0, len(messages))
	for _, message := range messages {
		messageIDStrs = append(messageIDStrs, strconv.FormatInt(message.MessageID, 10))
	}
	...
	messageExtras, err := m.messageExtraDB.queryWithMessageIDs(messageIDStrs)
	if err != nil {
		m.Error("查询消息扩展数据失败！", zap.Error(err))
		return
	}
	tx, err := m.ctx.DB().Begin()
	if err != nil {
	...
	for _, message := range messages {
		...
		fromUIDSet[message.FromUID] = struct{}{}
		...
		if err != nil {
			tx.RollbackUnlessCommitted()
			m.Error("更新消息已读数量失败！", zap.Error(err), zap.String("messageID", messageIDStr), zap.String("channelID", fakeChannelID))
			return
		}
	}
	if err := tx.Commit(); err != nil {
		tx.RollbackUnlessCommitted()
		m.Error("提交消息已读数量失败！", zap.Error(err))
		return
	}
	if reqChannelType == common.ChannelTypePerson.Uint8() {
		err = m.ctx.SendCMD(config.MsgCMDReq{
		...
		for fromUID := range fromUIDSet {
			fromUIDs = append(fromUIDs, fromUID)
		}
		err = m.ctx.SendCMD(config.MsgCMDReq{
			NoPersist:   true,
			Subscribers: fromUIDs,
			CMD:         common.CMDSyncMessageExtra,
			Param: map[string]interface{}{
				"channel_id":   fakeChannelID,
				"channel_type": reqChannelType,
			},
		})
	}
	if err != nil {
		m.Error("发送消息扩展同步命令失败！", zap.Error(err))
	}
}
...
func newSyncChannelMessageResp(resp *config.SyncChannelMessageResp, loginUID string, deviceUUID string, channelID string, channelType uint8, messageExtraDB *messageExtraDB, messageUserExtraDB *messageUserExtraDB, messageReactionDB *messageReactionDB, channelOffsetDB *channelOffsetDB, deviceOffsetDB *deviceOffsetDB, channelOffsetMessageSeq uint32) *syncChannelMessageResp {
	...
	if len(resp.Messages) > 0 {
		...
		for _, message := range resp.Messages {
			...
			if len(payloadMap) > 0 {
				if msgId, ok := replyMessageID(payloadMap); ok {
		...
		for _, message := range resp.Messages {
			...
			if len(payloadMap) > 0 {
				...
				msgId, ok := replyMessageID(payloadMap)
				if !ok {
					continue
				}
				for _, messageExtra := range messageExtras {
...
func replyMessageID(payloadMap map[string]interface{}) (string, bool) {
	replyJson := payloadMap["reply"]
	if replyJson == nil {
		return "", false
	}
	...
	messageID, ok := replyMap["message_id"]
	if !ok || messageID == nil {
		return "", false
	}
...
func (m *MsgSyncResp) from(msgResp *config.MessageResp, loginUID string, messageExtraM *messageExtraDetailModel, messageUserExtraM *messageUserExtraModel, reactionModels []*reactionModel, channelOffsetMessageSeq uint32) {
	defer func() {
		if err := recover(); err != nil {
			log.Error("消息转换失败，已降级为内容异常消息",
				zap.Any("error", err),
				zap.Int64("message_id", msgResp.MessageID),
				zap.Uint32("message_seq", msgResp.MessageSeq),
				zap.String("channel_id", msgResp.ChannelID),
				zap.Uint8("channel_type", msgResp.ChannelType),
				zap.String("payload", string(msgResp.Payload)),
			)
	...
	if setting.Signal {
		...
		if len(payloadMap) > 0 {
			...
			if visibles != nil {
				...
				if !ok {
					log.Warn("消息visibles字段格式错误，已忽略",
						zap.Int64("message_id", msgResp.MessageID),
						zap.Any("visibles", visibles),
					)
					visiblesArray = nil
				}
				if len(visiblesArray) > 0 {
					...
					for _, limitUID := range visiblesArray {
						if limitUIDStr, ok := limitUID.(string); ok && limitUIDStr == loginUID {
							m.IsDeleted = 0
						}
					}
				}
