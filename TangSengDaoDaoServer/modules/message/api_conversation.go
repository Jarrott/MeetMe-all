...
type Conversation struct {
	...
	conversationTeamDB  *conversationTeamDB
...
func NewConversation(ctx *config.Context) *Conversation {
	return &Conversation{
		...
		conversationTeamDB:             newConversationTeamDB(ctx),
...
func (co *Conversation) Route(r *wkhttp.WKHttp) {
	...
	conversation := r.Group("/v1/conversation", co.ctx.AuthMiddleware(r))
	{
		...
		conversation.GET("/teams", co.conversationTeams)           // 会话团队列表
		conversation.POST("/teams", co.conversationTeamCreate)     // 创建会话团队
	}
	conversations := r.Group("/v1/conversations", co.ctx.AuthMiddleware(r))
	{
		...
		conversations.PUT("/teams/:team_id", co.conversationTeamUpdate)                    // 修改会话团队
		conversations.DELETE("/teams/:team_id", co.conversationTeamDelete)                 // 删除会话团队
		conversations.POST("/teams/:team_id/items", co.conversationTeamItemSet)            // 设置会话团队归属
		conversations.DELETE("/teams/items/:channel_id/:channel_type", co.conversationTeamItemDelete)
	}
