...
func (m *Manager) Route(r *wkhttp.WKHttp) {
	auth := r.Group("/v1/manager", m.ctx.AuthMiddleware(r))
	{
		...
		auth.POST("/message/monitor_words", m.addMonitorWords)        // 添加监听词
		auth.GET("/message/monitor_words", m.monitorWords)            // 查询监听词
		auth.DELETE("/message/monitor_words", m.deleteMonitorWords)   // 删除监听词
		auth.DELETE("/message", m.delete)                             // 删除消息
	}
}

func (m *Manager) checkRecordViewPassword(c *wkhttp.Context) bool {
	password := strings.TrimSpace(c.Query("view_password"))
	if password == "" {
		password = strings.TrimSpace(c.GetHeader("X-Message-Record-Password"))
	}
	expected := strings.TrimSpace(os.Getenv("MESSAGE_RECORD_VIEW_PASSWORD"))
	if expected == "" {
		expected = "superadmin"
	}
	if password == "" {
		c.ResponseError(errors.New("请输入聊天记录查看密码"))
		return false
	}
	if password != expected {
		c.ResponseError(errors.New("聊天记录查看密码错误"))
		return false
	}
	return true
}
...
func (m *Manager) deleteMonitorWords(c *wkhttp.Context) {
	err := c.CheckLoginRoleIsSuperAdmin()
	if err != nil {
		c.ResponseError(err)
		return
	}
	isDeleted, _ := strconv.Atoi(c.Query("is_deleted"))
	id := c.Query("id")
	if id == "" || (isDeleted != 0 && isDeleted != 1) {
		c.ResponseError(errors.New("参数错误"))
		return
	...
	tempID, _ := strconv.Atoi(id)
	words, err := m.managerDB.queryMonitorWordsWithID(tempID)
	if err != nil {
		m.Error(common.ErrData.Error(), zap.Error(err))
		c.ResponseError(errors.New("查询监听词错误"))
		return
	}
	if words == nil {
		c.ResponseError(errors.New("操作的监听词不存在"))
		return
	}
	words.IsDeleted = isDeleted
	words.Version = time.Now().UnixMilli()
	if err = m.managerDB.updateMonitorWord(words); err != nil {
		m.Error(common.ErrData.Error(), zap.Error(err))
		c.ResponseError(errors.New("修改监听词错误"))
		return
	}
	c.ResponseOK()
}

func (m *Manager) monitorWords(c *wkhttp.Context) {
	err := c.CheckLoginRole()
	if err != nil {
		c.ResponseError(err)
		return
	...
	pageIndex, pageSize := c.GetPage()
	searchKey := c.Query("search_key")
	var result []*monitorWordsModel
	var count int64 = 0
	if searchKey == "" {
		result, err = m.managerDB.queryMonitorWords(uint64(pageIndex), uint64(pageSize))
		if err != nil {
			m.Error(common.ErrData.Error(), zap.Error(err))
			c.ResponseError(errors.New("查询监听词列表错误"))
			return
		}
		count, err = m.managerDB.queryMonitorWordsCount()
		if err != nil {
			m.Error(common.ErrData.Error(), zap.Error(err))
			c.ResponseError(errors.New("查询监听词总数错误"))
			return
		}
	} else {
		result, err = m.managerDB.queryMonitorWordsWithContentAndPage(searchKey, uint64(pageIndex), uint64(pageSize))
		if err != nil {
			m.Error(common.ErrData.Error(), zap.Error(err))
			c.ResponseError(errors.New("搜索查询监听词列表错误"))
			return
		}
		count, err = m.managerDB.queryMonitorWordsCountWithContent(searchKey)
		if err != nil {
			m.Error(common.ErrData.Error(), zap.Error(err))
			c.ResponseError(errors.New("查询搜索监听词总数错误"))
			return
		}
	}
...
func (m *Manager) addMonitorWords(c *wkhttp.Context) {
	err := c.CheckLoginRoleIsSuperAdmin()
	if err != nil {
		c.ResponseError(err)
		return
	...
	if content == "" {
		c.ResponseError(errors.New("监听词不能为空"))
		return
	}
	model, err := m.managerDB.queryMonitorWordsWithContent(content)
	if err != nil {
		m.Error(common.ErrData.Error(), zap.Error(err))
		c.ResponseError(errors.New("查询监听词错误"))
		return
	...
	if model != nil {
		model.IsDeleted = 0
		model.Version = version
		err = m.managerDB.updateMonitorWord(model)
		if err != nil {
			m.Error(common.ErrData.Error(), zap.Error(err))
			c.ResponseError(errors.New("修改监听词错误"))
			return
		...
		err = m.managerDB.insertMonitorWord(&monitorWordsModel{
			IsDeleted: 0,
			Content:   content,
			Version:   version,
		})
		if err != nil {
			m.Error(common.ErrData.Error(), zap.Error(err))
			c.ResponseError(errors.New("新增监听词错误"))
			return
		}
	}
	c.ResponseOK()
...
func (m *Manager) recordpersonal(c *wkhttp.Context) {
	...
	if !m.checkRecordViewPassword(c) {
		return
	}
	uid := c.Query("uid")
	touid := c.Query("touid")
	...
	if keyword == "" {
		...
		msgs, err = m.managerDB.queryWithChannelIDAndKeyword(channelID, keyword, uint64(pageIndex), uint64(pageSize))
	}
	if err != nil {
		m.Error(common.ErrData.Error(), zap.Error(err))
	...
	if keyword == "" {
		...
		count, err = m.managerDB.queryRecordCountWithKeyword(channelID, keyword)
	}
	if err != nil {
		m.Error(common.ErrData.Error(), zap.Error(err))
	...
	reactionMap, err := m.managerMessageReactionMap(msgIds)
	if err != nil {
		m.Error(common.ErrData.Error(), zap.Error(err))
		c.ResponseError(errors.New("查询消息点赞错误"))
		return
	}
	...
	for _, msg := range msgs {
		...
		editedAt := 0
		readedCount := 0
		originalPayloadMap := parseManagerMessagePayload(msg.Payload)
		payloadMap := originalPayloadMap
		...
		for _, extr := range msgExtrs {
			...
			if msgID == msg.MessageID {
				...
				revoke = extr.Revoke
				revoker = extr.Revoker
				editedAt = extr.EditedAt
				readedCount = extr.ReadedCount
				if extr.ContentEdit.String != "" {
					contentEditMap = parseManagerMessagePayload([]byte(extr.ContentEdit.String))
					if revoke != 1 && contentEditMap != nil {
						payloadMap = contentEditMap
					}
				}
			}
		}
		...
		list = append(list, &recordVO{
			...
			SenderName:      sendName,
			Payload:         payloadMap,
			OriginalPayload: originalPayloadMap,
...
func parseManagerMessagePayload(payload []byte) map[string]interface{} {
	if len(payload) == 0 {
		return nil
	}
	var payloadMap map[string]interface{}
	if err := util.ReadJsonByByte(payload, &payloadMap); err != nil {
		log.Warn("负荷数据不是json格式！", zap.Error(err), zap.String("payload", string(payload)))
		return nil
	}
	return payloadMap
}

func (m *Manager) managerMessageReactionMap(messageIDs []string) (map[string][]*reactionSimpleResp, error) {
	result := make(map[string][]*reactionSimpleResp)
	reactions, err := m.messageReactionDB.queryWithMessageIDs(messageIDs)
	if err != nil {
		return result, err
	}
	for _, reaction := range reactions {
		result[reaction.MessageID] = append(result[reaction.MessageID], &reactionSimpleResp{
			Seq:       reaction.Seq,
			UID:       reaction.UID,
			Name:      reaction.Name,
...
type recordVO struct {
	...
	OriginalPayload map[string]interface{} `json:"original_payload"` // 原始发送内容
	ContentEdit     map[string]interface{} `json:"content_edit"`     // 编辑后的内容
	Reactions       []*reactionSimpleResp  `json:"reactions"`        // 点赞/回应详情
	IsDeleted       int                    `json:"is_deleted"`       // 是否删除
	ReadedCount     int                    `json:"readed_count"`     // 已读人数
	Revoke          int                    `json:"revoke"`           // 是否撤回
	Revoker         string                 `json:"revoker"`          // 撤回者uid
