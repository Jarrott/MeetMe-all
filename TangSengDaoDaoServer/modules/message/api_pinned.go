...
func (m *Message) pinnedMessage(c *wkhttp.Context) {
	...
	if err != nil {
		c.ResponseError(errors.New("消息ID不合法"))
		return
	}
	syncMsg, err := m.ctx.IMGetWithChannelAndSeqs(req.ChannelID, req.ChannelType, loginUID, []uint32{req.MessageSeq})
	if err != nil {
	...
	if message.MessageID != messageID {
		c.ResponseError(errors.New("消息ID和消息seq不匹配"))
		return
	}
	...
	var maxCount = 10
	if appConfig != nil && appConfig.ChannelPinnedMessageMaxCount > 0 {
	...
	if isSendSystemMsg {
		...
		case json.Number:
			contentType, _ = v.Int64()
		case float64:
			contentType = int64(v)
		case int:
			contentType = int64(v)
		}
		if int(contentType) == common.Text.Int() {
			content, _ = payloadMap["content"].(string)
			content = fmt.Sprintf("`%s`", content)
		} else {
			content = common.GetDisplayText(int(contentType))
		}
