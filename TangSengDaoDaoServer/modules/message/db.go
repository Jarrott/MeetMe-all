...
func (d *DB) queryMessagesByIDs(channelID string, channelType uint8, messageIDs []int64) ([]*messageModel, error) {
	var list []*messageModel
	if len(messageIDs) == 0 {
		return list, nil
	}
	_, err := d.session.Select("message_id", "message_seq", "from_uid", "channel_id", "channel_type").From(d.getTable(channelID)).Where("channel_id=? and channel_type=? and message_id in ?", channelID, channelType, messageIDs).Load(&list)
	return list, err
}
...
func (d *DB) queryActiveProhibitWords() ([]*ProhibitWordModel, error) {
	var list []*ProhibitWordModel
	_, err := d.session.Select("*").From("prohibit_words").Where("is_deleted=0").Load(&list)
	return list, err
}

func (d *DB) queryActiveMonitorWords() ([]*MonitorWordModel, error) {
	var list []*MonitorWordModel
	_, err := d.session.Select("*").From("monitor_words").Where("is_deleted=0").Load(&list)
	return list, err
}
...
// MonitorWordModel 监听词model
type MonitorWordModel struct {
	Content   string
	IsDeleted int
	Version   int64
	db.BaseModel
