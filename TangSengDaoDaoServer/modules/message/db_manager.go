...
func (m *managerDB) queryWithChannelIDAndKeyword(channelID string, keyword string, page, pageSize uint64) ([]*messageModel, error) {
	var list []*messageModel
	var table = m.db.getTable(channelID)
	likeKeyword := "%" + keyword + "%"
	_, err := m.session.Select("message.*").From(table+" as message").
		LeftJoin("user", "message.from_uid=user.uid").
		Where("message.channel_id=? and (message.payload like ? or message.from_uid like ? or user.name like ?)", channelID, likeKeyword, likeKeyword, likeKeyword).
		Offset((page-1)*pageSize).
		Limit(pageSize).
		OrderDir("message.created_at", false).
		Load(&list)
	return list, err
}
...
func (m *managerDB) queryRecordCountWithKeyword(channelID string, keyword string) (int64, error) {
	var count int64
	var table = m.db.getTable(channelID)
	likeKeyword := "%" + keyword + "%"
	_, err := m.session.Select("count(*)").From(table+" as message").
		LeftJoin("user", "message.from_uid=user.uid").
		Where("message.channel_id=? and (message.payload like ? or message.from_uid like ? or user.name like ?)", channelID, likeKeyword, likeKeyword, likeKeyword).
		Load(&count)
	return count, err
...
func (m *managerDB) queryMonitorWordsWithContent(content string) (*monitorWordsModel, error) {
	var model *monitorWordsModel
	_, err := m.session.Select("*").From("monitor_words").Where("content=?", content).Load(&model)
	return model, err
}

func (m *managerDB) queryMonitorWordsWithID(id int) (*monitorWordsModel, error) {
	var model *monitorWordsModel
	_, err := m.session.Select("*").From("monitor_words").Where("id=?", id).Load(&model)
	return model, err
}

func (m *managerDB) updateMonitorWord(word *monitorWordsModel) error {
	_, err := m.session.Update("monitor_words").SetMap(map[string]interface{}{
		"version":    word.Version,
		"is_deleted": word.IsDeleted,
	}).Where("content=?", word.Content).Exec()
	return err
...
func (m *managerDB) insertMonitorWord(word *monitorWordsModel) error {
	_, err := m.session.InsertInto("monitor_words").Columns(util.AttrToUnderscore(word)...).Record(word).Exec()
	return err
}

func (m *managerDB) queryMonitorWords(pageIndex, pageSize uint64) ([]*monitorWordsModel, error) {
	var list []*monitorWordsModel
	_, err := m.session.Select("*").From("monitor_words").Offset((pageIndex-1)*pageSize).Limit(pageSize).OrderDir("created_at", false).Load(&list)
	return list, err
}

func (m *managerDB) queryMonitorWordsWithContentAndPage(content string, pageIndex, pageSize uint64) ([]*monitorWordsModel, error) {
	var list []*monitorWordsModel
	_, err := m.session.Select("*").From("monitor_words").Where("content like ?", "%"+content+"%").Offset((pageIndex-1)*pageSize).Limit(pageSize).OrderDir("created_at", false).Load(&list)
	return list, err
}

func (m *managerDB) queryMonitorWordsCount() (int64, error) {
	var count int64
	_, err := m.session.Select("count(*)").From("monitor_words").Load(&count)
	return count, err
}

func (m *managerDB) queryMonitorWordsCountWithContent(content string) (int64, error) {
	var count int64
	_, err := m.session.Select("count(*)").From("monitor_words").Where("content like ?", "%"+content+"%").Load(&count)
	return count, err
}
