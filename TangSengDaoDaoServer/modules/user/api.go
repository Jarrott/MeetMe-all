...
import (
	...
	dicebearstyles "github.com/dicebear/styles/v10"
...
type User struct {
	...
	emailService  commonapi.IEmailService
...
func New(ctx *config.Context) *User {
	u := &User{
		...
		emailService:             commonapi.NewEmailService(ctx),
...
func (u *User) Route(r *wkhttp.WKHttp) {
	...
	user := r.Group("/v1/user", u.ctx.AuthMiddleware(r))
	{
		...
		user.POST("/email/bind/code", u.sendBindEmailCode)         // 发送绑定/换绑邮箱验证码
		user.POST("/email/bind", u.bindEmail)                      // 绑定/换绑邮箱
		user.POST("/phone/bind/code", u.sendBindPhoneCode)         // 发送绑定/换绑手机号验证码
		user.POST("/phone/bind", u.bindPhone)                      // 绑定/换绑手机号
		// #################### 登录设备管理 ####################
	...
	v := r.Group("/v1")
	{
		...
		v.POST("/user/email/login", u.emailLogin)            // 邮箱登录
		v.POST("/user/email/register", u.emailRegister)      // 邮箱注册
		v.POST("/user/email/registercode", u.sendEmailRegisterCode)
		v.POST("/user/email/register_code", u.sendEmailRegisterCode)
		v.POST("/user/email/code", u.sendEmailRegisterCode)
...
func (u *User) UserAvatar(c *wkhttp.Context) {
	...
	if userInfo.IsUploadAvatar == 1 {
		...
		// 未上传头像的用户优先使用本地默认头像，避免依赖第三方头像服务不可用。
		avatarPath := strings.TrimSpace(u.ctx.GetConfig().Avatar.Default)
		if avatarPath == "" {
			avatarPath = "assets/assets/avatar.png"
		}
		imageData, err := os.ReadFile(avatarPath)
		if err != nil {
			u.Error("打开本地头像文件失败", zap.String("path", avatarPath), zap.Error(err))
		} else {
...
func normalizeIMRouteAddrs(resultMap map[string]interface{}) {
	wsAddr, _ := resultMap["ws_addr"].(string)
	wssAddr, _ := resultMap["wss_addr"].(string)
	if wssAddr == "" && strings.HasPrefix(wsAddr, "wss://") {
		resultMap["wss_addr"] = wsAddr
	}
	if wsAddr == "" && wssAddr != "" {
		resultMap["ws_addr"] = wssAddr
	}
}
...
func (u *User) execLogin(userInfo *Model, flag config.DeviceFlag, device *deviceReq, loginSpanCtx context.Context) (*loginUserDetailResp, error) {
	...
	if userInfo.RegisterAuditStatus == int(RegisterAuditPending) {
		return nil, errors.New("账号注册审核中，请等待后台审核通过后再登录")
	}
	if userInfo.RegisterAuditStatus == int(RegisterAuditRejected) {
		if strings.TrimSpace(userInfo.AuditRemark) != "" {
			return nil, errors.New("账号注册审核未通过：" + userInfo.AuditRemark)
		}
		return nil, errors.New("账号注册审核未通过")
	}
...
func (u *User) createUserWithRespAndTx(registerSpanCtx context.Context, createUser *createUserModel, publicIP string, invite *model.Invite, tx *dbr.Tx, commitCallback func() error) (*loginUserDetailResp, error) {
	...
	if userModel.IsUploadAvatar == 0 {
		err = u.uploadDicebearDefaultAvatar(userModel.UID)
		if err != nil {
			u.Warn("生成默认头像失败，将使用本地默认头像", zap.Error(err), zap.String("uid", userModel.UID))
		} else {
			userModel.IsUploadAvatar = 1
		}
	}
	...
	userModel.RegisterAuditStatus = int(RegisterAuditPending)
	...
	if userModel.RegisterAuditStatus != int(RegisterAuditApproved) {
		u.insertRegisterAuditAlert(userModel)
		if u.ctx.GetConfig().ShortNo.NumOn {
		...
		return newLoginUserDetailResp(userModel, "", u.ctx), nil
	}
...
func (u *User) insertRegisterAuditAlert(userModel *Model) {
	content := fmt.Sprintf("新用户 %s 注册后等待审核", userModel.Name)
	if strings.TrimSpace(userModel.Name) == "" {
		content = fmt.Sprintf("新用户 %s 注册后等待审核", userModel.Username)
	}
	err := common2.InsertManagerAlert(u.db.session, &common2.ManagerAlertModel{
		AlertType:  "register_audit",
		Title:      "新注册用户待审核",
		Content:    content,
		ActorUID:   userModel.UID,
		ActorName:  userModel.Name,
		TargetID:   userModel.UID,
		TargetType: "user",
		IsRead:     0,
		Payload: util.ToJson(map[string]interface{}{
			...
			"zone":                  userModel.Zone,
			"register_audit_status": userModel.RegisterAuditStatus,
		}),
	})
	if err != nil {
		u.Warn("写入注册审核后台提醒失败", zap.Error(err), zap.String("uid", userModel.UID))
	}
}

func (u *User) uploadDicebearDefaultAvatar(uid string) error {
	style, err := dicebear.NewStyle([]byte(dicebearstyles.Micah))
	if err != nil {
		return err
	}
	...
	avatarID := crc32.ChecksumIEEE([]byte(uid)) % uint32(u.ctx.GetConfig().Avatar.Partition)
	_, err = u.fileService.UploadFile(fmt.Sprintf("avatar/%d/%s.png", avatarID, uid), "image/svg+xml", func(w io.Writer) error {
		_, err := io.Copy(w, strings.NewReader(avatar.SVG()))
		return err
	})
	return err
}
...
type loginUserDetailResp struct {
	...
	RegisterAuditStatus int     `json:"register_audit_status"` // 注册审核状态 0.待审核 1.通过 2.拒绝
	AuditRemark         string  `json:"audit_remark"`          // 注册审核备注
}
...
func newLoginUserDetailResp(m *Model, token string, ctx *config.Context) *loginUserDetailResp {
	...
	return &loginUserDetailResp{
		...
		RegisterAuditStatus: m.RegisterAuditStatus,
