...
func (m *Manager) Route(r *wkhttp.WKHttp) {
	...
	auth := r.Group("/v1/manager", m.ctx.AuthMiddleware(r))
	{
		...
		auth.GET("/user/auditlist", m.auditUsers)             // 注册审核用户列表
		...
		auth.PUT("/user/audit/:uid", m.auditUser)             // 审核注册用户
		auth.PUT("/user/profile/:uid", m.updateUserProfile)   // 修改用户登录账号和后台备注
...
func (m *Manager) addAdminUser(c *wkhttp.Context) {
	...
	userModel.RegisterAuditStatus = int(RegisterAuditApproved)
	...
	if len(userList) > 0 {
		...
		for _, user := range userList {
			...
			result = append(result, &managerUserResp{
				...
				RegisterTime:        user.CreatedAt.String(),
				Status:              user.Status,
				RegisterAuditStatus: user.RegisterAuditStatus,
...
// 查看注册审核用户列表
func (m *Manager) auditUsers(c *wkhttp.Context) {
	err := c.CheckLoginRole()
	if err != nil {
		c.ResponseError(err)
		return
	}
	status := int(RegisterAuditPending)
	statusStr := c.Query("status")
	if strings.TrimSpace(statusStr) != "" {
		status, _ = strconv.Atoi(statusStr)
	}
	if !validRegisterAuditStatus(status) {
		c.ResponseError(errors.New("审核状态不匹配"))
		return
	}
	pageIndex, pageSize := c.GetPage()
	list, err := m.db.queryUserListWithRegisterAuditStatus(status, uint64(pageSize), uint64(pageIndex))
	if err != nil {
		m.Error("通过审核状态查询用户列表错误", zap.Error(err))
		c.ResponseError(errors.New("通过审核状态查询用户列表错误"))
		return
	}
	count, err := m.db.queryUserCountWithRegisterAuditStatus(status)
	if err != nil {
		m.Error("查询审核用户数量错误", zap.Error(err))
		c.ResponseError(errors.New("查询审核用户数量错误"))
		return
	}
	result := make([]*managerAuditUserResp, 0)
	for _, user := range list {
		result = append(result, &managerAuditUserResp{
			Name:                user.Name,
			UID:                 user.UID,
			Username:            user.Username,
			Email:               user.Email,
			...
			Sex:                 user.Sex,
			RegisterTime:        user.CreatedAt.String(),
			Phone:               getShowPhoneNum(user.Phone),
			RegisterAuditStatus: user.RegisterAuditStatus,
			AuditRemark:         user.AuditRemark,
		})
	}
...
// 审核注册用户
func (m *Manager) auditUser(c *wkhttp.Context) {
	err := c.CheckLoginRole()
	if err != nil {
		c.ResponseError(err)
		return
	...
	auditStatus := userInfo.RegisterAuditStatus
	if req.Status != nil {
		if !validRegisterAuditStatus(*req.Status) {
			c.ResponseError(errors.New("审核状态不匹配"))
			return
		}
		auditStatus = *req.Status
	}
	err = m.userDB.updateUser(map[string]interface{}{
		"register_audit_status": auditStatus,
		"audit_remark":          remark,
	}, uid)
	if err != nil {
		m.Error("更新用户审核状态错误", zap.Error(err), zap.String("uid", uid))
		c.ResponseError(errors.New("更新用户审核状态错误"))
		return
	}
	if auditStatus != int(RegisterAuditApproved) {
...
// 修改用户登录账号、邮箱、手机号和后台备注
func (m *Manager) updateUserProfile(c *wkhttp.Context) {
	err := c.CheckLoginRole()
	if err != nil {
		c.ResponseError(err)
		return
	...
	var req managerUpdateUserProfileReq
	if err := c.BindJSON(&req); err != nil {
		c.ResponseError(errors.New("请求数据格式有误！"))
		return
	}
	...
	if username == "" {
		c.ResponseError(errors.New("登录账号不能为空"))
		return
	}
	if len([]rune(username)) > 64 {
		c.ResponseError(errors.New("登录账号不能超过64个字符"))
		return
	}
	...
	adminRemark := strings.TrimSpace(req.AdminRemark)
	if len([]rune(adminRemark)) > 500 {
		adminRemark = string([]rune(adminRemark)[:500])
	}
	userInfo, err := m.userDB.QueryByUID(uid)
	if err != nil {
		m.Error("查询用户信息失败！", zap.String("uid", uid), zap.Error(err))
	...
	existUser, err := m.userDB.QueryByUsername(username)
	if err != nil {
		m.Error("查询登录账号是否重复失败！", zap.String("username", username), zap.Error(err))
		c.ResponseError(errors.New("查询登录账号是否重复失败"))
		return
	}
	if existUser != nil && existUser.UID != "" && existUser.UID != uid {
		c.ResponseError(errors.New("登录账号已存在"))
		return
	}
	if email != "" {
		existEmailUser, err := m.userDB.QueryByUsername(email)
		if err != nil {
			m.Error("查询邮箱是否重复失败！", zap.String("email", email), zap.Error(err))
			c.ResponseError(errors.New("查询邮箱是否重复失败"))
			return
		}
		if existEmailUser != nil && existEmailUser.UID != "" && existEmailUser.UID != uid {
			c.ResponseError(errors.New("邮箱已被绑定"))
			return
		}
	}
	if phone != "" {
		existPhoneUser, err := m.userDB.QueryByPhone(zone, phone)
		if err != nil {
			m.Error("查询手机号是否重复失败！", zap.String("zone", zone), zap.String("phone", phone), zap.Error(err))
			c.ResponseError(errors.New("查询手机号是否重复失败"))
			return
		}
		if existPhoneUser != nil && existPhoneUser.UID != "" && existPhoneUser.UID != uid {
			c.ResponseError(errors.New("手机号已被绑定"))
			return
		}
	}
	...
	err = m.userDB.updateUser(updateMap, uid)
	if err != nil {
		m.Error("更新用户资料错误", zap.Error(err), zap.String("uid", uid))
		c.ResponseError(errors.New("更新用户资料错误"))
		return
	}
	if userInfo.Username != username || userInfo.Email != email || userInfo.Zone != zone || userInfo.Phone != phone {
		err = m.ctx.QuitUserDevice(userInfo.UID, -1)
		if err != nil {
			m.Error("下线用户所有登录设备错误", zap.Error(err), zap.String("uid", uid))
			c.ResponseError(errors.New("下线用户所有登录设备错误"))
...
type managerAuditUserReq struct {
	Status *int   `json:"status"`
	Remark string `json:"remark"`
}
type managerUpdateUserProfileReq struct {
	Username    string `json:"username"`
...
type managerUserResp struct {
	...
	Status              int    `json:"status"`
	RegisterAuditStatus int    `json:"register_audit_status"`
...
type managerAuditUserResp struct {
	Name                string `json:"name"`
	UID                 string `json:"uid"`
	Username            string `json:"username"`
	ShortNo             string `json:"short_no"`
...
func validRegisterAuditStatus(status int) bool {
	return status == int(RegisterAuditPending) || status == int(RegisterAuditApproved) || status == int(RegisterAuditRejected)
}
