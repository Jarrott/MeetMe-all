...
func TestAuditUser(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/manager/user/auditlist?page_index=1&page_size=10", nil)
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"name":"audit_user"`))
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"register_audit_status":0`))

	w = httptest.NewRecorder()
	req, _ = http.NewRequest("PUT", fmt.Sprintf("/v1/manager/user/audit/%s", testutil.UID), bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"status": int(RegisterAuditApproved),
		"remark": "资料完整",
	}))))
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
	...
	userInfo, err := m.userDB.QueryByUID(testutil.UID)
	assert.NoError(t, err)
	assert.Equal(t, int(RegisterAuditApproved), userInfo.RegisterAuditStatus)
	assert.Equal(t, "资料完整", userInfo.AuditRemark)
}
