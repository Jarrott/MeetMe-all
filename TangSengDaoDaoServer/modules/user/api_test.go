...
func TestUser_Register(t *testing.T) {
	...
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"token":""`))
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"register_audit_status":0`))
...
func TestEmailRegister(t *testing.T) {
	...
	err := testutil.CleanAllTables(ctx)
	assert.NoError(t, err)
	email := "newuser@meetme.im"
	code := "654321"
	err = ctx.GetRedisConn().SetAndExpire(fmt.Sprintf("%s%d@%s", commonapi.CacheKeyEmailCode, commonapi.CodeTypeRegister, email), code, time.Minute*5)
	assert.NoError(t, err)

	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/user/email/register", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"email":    "NewUser@MeetMe.im",
		"name":     "Email User",
		"code":     code,
		"password": "123123",
		"device": map[string]interface{}{
			"device_id":    "email_device_id1",
			"device_name":  "device_name1",
			"device_model": "device_model1",
		},
	}))))
	...
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"email":"newuser@meetme.im"`))
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"token":""`))
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"register_audit_status":0`))
}

func TestEmailRegisterNeedCode(t *testing.T) {
	s, ctx := testutil.NewTestServer()
	err := testutil.CleanAllTables(ctx)
	assert.NoError(t, err)
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/user/email/register", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"email":    "needcode@meetme.im",
		"name":     "Email User",
		"password": "123123",
		"device": map[string]interface{}{
			"device_id":    "email_device_need_code",
			"device_name":  "device_name1",
			"device_model": "device_model1",
		},
	}))))
	...
	assert.Equal(t, true, strings.Contains(w.Body.String(), "验证码不能为空"))
}
...
func TestEmailLogin(t *testing.T) {
	...
	err = u.db.Insert(&Model{
		UID:      "email_login_user",
		Email:    "login@meetme.im",
		Password: util.MD5(util.MD5(password)),
		Name:     "Email Login",
		ShortNo:  "email_login_user",
		Status:   1,
	})
	assert.NoError(t, err)

	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/user/email/login", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"email":    "LOGIN@meetme.im",
		"password": password,
		"device": map[string]interface{}{
			"device_id":    "email_device_id2",
			"device_name":  "device_name1",
			"device_model": "device_model1",
		},
	}))))
	...
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"token":`))
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"email":"login@meetme.im"`))
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"register_audit_status":1`))
}
...
func TestBindEmail(t *testing.T) {
	...
	err = u.db.Insert(&Model{
		...
		Zone:     "0086",
		Phone:    "13600000002",
		Username: "008613600000002",
		Password: util.MD5(util.MD5("123123")),
		Name:     "Bind Email",
		ShortNo:  "bind_email_user",
		Status:   1,
	})
	assert.NoError(t, err)

	email := "bind_email@meetme.im"
	code := "234567"
	err = ctx.GetRedisConn().SetAndExpire(fmt.Sprintf("%s%d@%s", commonapi.CacheKeyEmailCode, commonapi.CodeTypeBindEmail, email), code, time.Minute*5)
	assert.NoError(t, err)

	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/user/email/bind", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"email": "BIND_EMAIL@MeetMe.im",
		"code":  code,
	}))))
	...
	assert.Equal(t, testutil.UID, userInfo.UID)
...
func TestBindPhone(t *testing.T) {
	...
	err = u.db.Insert(&Model{
		UID:      testutil.UID,
		Email:    "bind_phone@meetme.im",
		Password: util.MD5(util.MD5("123123")),
		Name:     "Bind Phone",
		ShortNo:  "bind_phone_user",
		Status:   1,
	})
	assert.NoError(t, err)
	...
	phone := "13700000002"
	code := "3456"
	err = ctx.GetRedisConn().SetAndExpire(fmt.Sprintf("%s%d@%s@%s", commonapi.CacheKeySMSCode, commonapi.CodeTypeBindPhone, zone, phone), code, time.Minute*5)
	assert.NoError(t, err)

	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/user/phone/bind", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"zone":  zone,
		"phone": phone,
		"code":  code,
	}))))
	...
	assert.Equal(t, http.StatusOK, w.Code)
	userInfo, err := u.db.QueryByUsername(zone + phone)
	assert.NoError(t, err)
	assert.NotNil(t, userInfo)
	assert.Equal(t, testutil.UID, userInfo.UID)
	assert.Equal(t, zone, userInfo.Zone)
	assert.Equal(t, phone, userInfo.Phone)
	assert.Equal(t, zone+phone, userInfo.Username)
}

func TestUserLoginNeedsRegisterAuditApproved(t *testing.T) {
	s, ctx := testutil.NewTestServer()
	u := New(ctx)
	err := testutil.CleanAllTables(ctx)
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	err = u.db.updateUser(map[string]interface{}{
		"register_audit_status": int(RegisterAuditPending),
	}, "audit_user")
	assert.NoError(t, err)
	...
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, true, strings.Contains(w.Body.String(), "账号注册审核中"))

	err = u.db.updateUser(map[string]interface{}{
		"register_audit_status": int(RegisterAuditApproved),
	}, "audit_user")
	assert.NoError(t, err)
	w = httptest.NewRecorder()
	req, _ = http.NewRequest("POST", "/v1/user/usernamelogin", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
