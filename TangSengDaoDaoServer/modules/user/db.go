...
func (d *DB) Insert(m *Model) error {
	if m.RegisterAuditStatus == 0 {
		m.RegisterAuditStatus = int(RegisterAuditApproved)
	}
...
func (d *DB) updateUserTx(userMap map[string]interface{}, uid string, tx *dbr.Tx) error {
	_, err := tx.Update("user").SetMap(userMap).Where("uid=?", uid).Exec()
	return err
}
...
type Model struct {
	...
	RegisterAuditStatus int    // 注册审核状态 0.待审核 1.通过 2.拒绝
	AuditRemark         string // 注册审核备注
