...
func (m *managerDB) queryUserListWithPage(pageSize, page uint64, onelineStatus int) ([]*managerUserModel, error) {
	...
	var users []*managerUserModel
	selectStm := m.session.Select("user.uid,user.name,user.username,user.email,user.status,user.register_audit_status,user.audit_remark,user.admin_remark,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid,max(user_online.online) online").From("user").LeftJoin("user_online", "user.uid=user_online.uid")
	if onelineStatus != -1 {
		selectStm = selectStm.Where("user_online.online=?", onelineStatus)
	}
	selectStm = selectStm.GroupBy("user.uid,user.name,user.username,user.email,user.status,user.register_audit_status,user.audit_remark,user.admin_remark,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid")
...
func (m *managerDB) queryUserListWithPageAndKeyword(keyword string, onelineStatus int, pageSize, page uint64) ([]*managerUserModel, error) {
	var users []*managerUserModel
	selectStm := m.session.Select("user.uid,user.name,user.username,user.email,user.status,user.register_audit_status,user.audit_remark,user.admin_remark,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid,max(user_online.online) online").From("user").LeftJoin("user_online", "user.uid=user_online.uid").Where("user.name like ? or user.uid like ? or user.phone like ? or user.email like ? or user.short_no like ? or user.username like ? or user.admin_remark like ?", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%")
	if onelineStatus != -1 {
		selectStm = selectStm.Where("user_online.online=?", onelineStatus)
	}
	selectStm = selectStm.GroupBy("user.uid,user.name,user.username,user.email,user.status,user.register_audit_status,user.audit_remark,user.admin_remark,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid")
...
func (m *managerDB) queryUserCountWithKeyWord(keyword string) (int64, error) {
	var count int64
	_, err := m.session.Select("count(*)").From("user").Where("name like ? or uid like ? or phone like ? or email like ? or short_no like ? or username like ? or admin_remark like ?", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%", "%"+keyword+"%").Load(&count)
	return count, err
}
...
// 通过注册审核状态查询用户列表
func (m *managerDB) queryUserListWithRegisterAuditStatus(status int, pageSize, page uint64) ([]*managerUserModel, error) {
	var users []*managerUserModel
	_, err := m.session.Select("*").From("user").Where("register_audit_status=?", status).Offset((page-1)*pageSize).Limit(pageSize).OrderDir("updated_at", false).Load(&users)
	return users, err
}

// 通过注册审核状态查询用户数量
func (m *managerDB) queryUserCountWithRegisterAuditStatus(status int) (int64, error) {
	var count int64
	_, err := m.session.Select("count(*)").From("user").Where("register_audit_status=?", status).Load(&count)
	return count, err
}
