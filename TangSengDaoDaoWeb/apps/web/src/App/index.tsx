import { ChatPage, EndpointCategory, WKApp, Menus, Wallet, RTCCallModal } from '@tsdaodao/base';
import { ContactsList } from '@tsdaodao/contacts';
import { IconMoneyExchangeStroked } from '@douyinfe/semi-icons';
...
async function registerMenus() {
  ...
  WKApp.menus.register("chat", (_context) => {
    ...
    const desktopIpc = (window as any).ipc;
    if (desktopIpc?.send) {
      desktopIpc.send("conversation-anager-unread-count", badge);
    }
  ...
  WKApp.menus.register("wallet", (_param) => {
    const WalletIcon = IconMoneyExchangeStroked as any;
    return new Menus("wallet", "/wallet", "钱包",
      <span className="wk-main-sider-menu-icon">{React.createElement(WalletIcon)}</span>,
      <span className="wk-main-sider-menu-icon selected">{React.createElement(WalletIcon)}</span>)
  }, 5000)
