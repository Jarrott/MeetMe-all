...
import { EndpointCategory, MessageContentTypeConst } from "./Service/Const";
...
export class WKRemoteConfig {
  ...
  autoTranslateOn: boolean = false; // 自动翻译
  conversationTeamEnabled: boolean = true; // 会话分组管理
  groupMemberPrivateChatEnabled: boolean = true; // 群成员不加好友私聊
  ...
  requestConfig() {
    return WKApp.apiClient.get("common/appconfig").then((result) => {
      ...
      this.autoTranslateOn = Number(result["auto_translate_on"]) === 1;
      this.conversationTeamEnabled = Number(result["conversation_team_enabled"] ?? 1) === 1;
      this.groupMemberPrivateChatEnabled = Number(result["group_member_private_chat_enabled"] ?? 1) === 1;
    });
  }
}
...
export default class WKApp extends ProviderListener {
  ...
  supportEdit = [MessageContentType.text, MessageContentType.image, MessageContentTypeConst.imageGroup, MessageContentTypeConst.smallVideo]; // 注册编辑的消息
  notSupportForward: number[] = []; // 不支持转发的消息

  openChannel?: Channel; // 当前打开的会话频道
  content?: JSX.Element;

  baseContext!: WKBaseContext; // MeetMe基础上下文
