...
export class Conversation extends Component<ConversationProps> implements ConversationContext {
    ...
    dragResetTimer: number | null = null
    readReceiptTimer: number | null = null
    updateBrowseToMessageSeqAndReminderDoneing: boolean = false
    private _dragFileCallback?: (files: File[] | FileList) => void
    ...
    reply(message: Message, handlerType: number): void {
        ...
        if (handlerType === 2) {
            let content = this.editableMessageText(message)
    ...
    editableMessageText(message: Message) {
        const content: any = message.remoteExtra?.isEdit ? message.remoteExtra?.contentEdit : message.content
        if (!content) {
            return ""
        }
        if (message.contentType === MessageContentType.text) {
            return content.text || content.content || content.conversationDigest || ""
        }
        if (this.isCaptionEditableMessage(message)) {
            return content.caption || content.text || ""
        }
        return content.conversationDigest || ""
    }

    isCaptionEditableMessage(message: Message) {
        return message.contentType === MessageContentType.image ||
            message.contentType === MessageContentTypeConst.imageGroup ||
            message.contentType === MessageContentTypeConst.smallVideo
    }

    buildEditContent(message: Message, text: string) {
        if (message.contentType === MessageContentType.text) {
        ...
        const sourceContent: any = message.remoteExtra?.isEdit ? message.remoteExtra?.contentEdit : message.content
        const json = sourceContent?.encodeJSON ? sourceContent.encodeJSON() : { ...(sourceContent || {}) }
        json['type'] = message.contentType
        json['caption'] = text
        json['text'] = text
        const editedContent = WKSDK.shared().getMessageContent(message.contentType) as any
        const payloadData = new TextEncoder().encode(JSON.stringify(json))
        editedContent.decode(payloadData)
        return { content: editedContent, json }
    }

    setDragFileCallback(f: (files: File[] | FileList) => void): void {
    ...
    syncPinnedMessages(): void {
        this.vm.syncPinnedMessages(0)
    }
    ...
    clearDraft(): void {
        if (this.vm.currentConversation?.remoteExtra) {
    ...
    componentDidMount() {
        ...
        this.vm.onMessageAppended = () => {
            this.scheduleUploadReaded()
        }

        window.addEventListener("dragend", this.handleGlobalDragEnd)
        window.addEventListener("drop", this.handleGlobalDragEnd)
        window.addEventListener("blur", this.handleGlobalDragEnd)
    ...
    componentWillUnmount() {
        ...
        window.removeEventListener("blur", this.handleGlobalDragEnd)
        ...
        if (this.vm) {
            this.vm.onMessageAppended = undefined
        }
        this.dealloc()
    }
    dealloc() {
    ...
    pinnedMessageUI(vm: ConversationVM) {
        const pinnedMessage = vm.currentPinnedMessage()
        if (!pinnedMessage) {
            return null
        }
        const digest = pinnedMessage.remoteExtra?.isEdit ? pinnedMessage.remoteExtra?.contentEdit?.conversationDigest : pinnedMessage.content?.conversationDigest
        return <div className="wk-conversation-pinned-section">
            <div className="wk-conversation-pinnedbar" onClick={() => {
                this.locateMessage(pinnedMessage.messageSeq)
            }}>
                <div className="wk-conversation-pinnedbar-mark"></div>
                <div className="wk-conversation-pinnedbar-content">
                    <div className="wk-conversation-pinnedbar-title">置顶</div>
                    <div className="wk-conversation-pinnedbar-digest">{digest || "[消息]"}</div>
                </div>
                <button className="wk-conversation-pinnedbar-close" onClick={(event) => {
                    event.stopPropagation()
                    WKApp.dataSource.channelDataSource.pinnedMessage(pinnedMessage).then(() => {
                        vm.syncPinnedMessages(0)
                    }).catch((err) => {
                        Toast.error(err.msg)
                    })
                }}>×</button>
    ...
    scheduleUploadReaded() {
        ...
        this.readReceiptTimer = window.setTimeout(() => {
            this.readReceiptTimer = null
            this.uploadReadedIfNeed()
        }, 500)
    }
        ...
        if (visiableMessages && visiableMessages.length > 0) {
            ...
            if (unreadMessages.length > 0) {
                ...
                unreadMessages.forEach((message) => {
                    message.remoteExtra.readed = true
                })
                this.vm.notifyListener()
            }
        }
    ...
    allVisiableMessages(vp: HTMLElement | null): Array<MessageWrap> {
        ...
        for (let index = 0; index < this.vm.messages.length; index++) {
            ...
            const element = document.getElementById(message.clientMsgNo)
            if (element) {
                const messageMiddle = element.offsetTop + element.clientHeight / 2;
                if (messageMiddle > viewportBottom) {
                    break
                }
                if (messageMiddle > targetScrollTop) { // message 要漏出来一半才算可见
    ...
    render() {
        ...
        return <Provider create={() => {
            ...
            return <>
                <div className={classNames("wk-conversation", vm.fileDragEnter ? "wk-conversation-dragover" : undefined, vm.currentReplyMessage ? "wk-conversation-hasreply" : undefined, vm.currentPinnedMessage() ? "wk-conversation-haspinned" : undefined)} style={{ "background": chatBg ? `url(${chatBg}) rgb(245, 247, 249)` : undefined }} onDragOver={(event) => {
                    ...
                    <div onDragOver={(event) => {
                        ...
                        {
                            vm.currentPinnedMessage() ? <div className="wk-conversation-chat-divider">
                                <span>聊天</span>
                            </div> : null
                        }
                        <div className={classNames("wk-conversation-messages", vm.currentPinnedMessage() ? "wk-conversation-messages-with-divider" : undefined)} id={vm.messageContainerId} onScroll={this.handleScroll.bind(this)}>
                            {
                            ...
                            <MessageInput members={this.vm.subscribers.filter((s) => s.uid !== WKApp.loginInfo.uid)} onContext={(ctx) => {
                                ...
                                if (vm.currentReplyMessage) {
                                    if (vm.currentHandlerType === 2) {
                                        // 编辑消息
                                        const msg = vm.currentReplyMessage as any
                                        const messageID = `${msg.messageIDStr || msg.message_idstr || msg.message_id_str || vm.currentReplyMessage.messageID || ""}`
                                        const edit = this.buildEditContent(vm.currentReplyMessage, text)
                                        await vm.editMessage(messageID, vm.currentReplyMessage.messageSeq, vm.currentReplyMessage.channel.channelID, vm.currentReplyMessage.channel.channelType, JSON.stringify(edit.json))
                                        vm.currentReplyMessage.remoteExtra.isEdit = true
                                        vm.currentReplyMessage.remoteExtra.contentEdit = edit.content
                                        vm.currentReplyMessage.remoteExtra.editedAt = Math.floor(Date.now() / 1000)
                                        vm.currentReplyMessage = undefined
                                        vm.notifyListener()
                                        return
                                    }
                ...
                <ContextMenus onContext={(ctx) => {
                    ...
                    ...(this.vm.channel.channelType === ChannelTypeGroup && this.vm.selectUID !== WKApp.loginInfo.uid && WKApp.remoteConfig.groupMemberPrivateChatEnabled ? [{
                        title: "发送信息",
                        onClick: () => {
                            if (!this.vm.selectUID) {
                                return
                            }
                            WKApp.endpoints.showConversation(new Channel(this.vm.selectUID, ChannelTypePerson))
                        }
                    }] : []),
                    {
                        title: "查看用户信息",
