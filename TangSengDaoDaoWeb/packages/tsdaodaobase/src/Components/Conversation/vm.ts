...
const MAX_LOADED_ORIGIN_MESSAGES = 500;
...
export default class ConversationVM extends ProviderListener {
  ...
  pinnedMessageMap: Map<string, any> = new Map();
  pinnedContentMap: Map<string, Message> = new Map();
  pinnedVersion: number = 0;
  ...
  onMessageAppended?: (message: Message) => void; // 当前会话收到新消息后触发
  ...
  markUnread() {
    if (this.needSetUnread) {
      WKApp.conversationProvider.markConversationUnread(
        this.channel,
        this.unreadCount
      );
    }
  ...
  sendMergeforward(toChannels: Channel[]) {
    ...
    let checkedMessages = this.getCheckedMessages().map(
  ...
  async editMessage(
    ...
    return WKApp.conversationProvider.editMessage(
      messageID,
      messageSeq,
      channelID,
      channelType,
  ...
  async deleteMessagesFromLocal(deletedMessages: Message[]): Promise<void> {
    ...
    for (const message of messages) {
      ...
      for (const deletedMessage of deletedMessages) {
        if (deletedMessage.clientMsgNo === message.clientMsgNo) {
          exist = true;
          this.removeSendingMessageIfNeed(
            deletedMessage.clientSeq,
            deletedMessage.channel
          );
          break;
        }
      }
    ...
    const lastMessageWrap = this.findLastDisplayMessage(newMessages);
    if (lastMessageWrap) {
      lastMessage = lastMessageWrap.message;
    }
    this.lastMessage = lastMessageWrap;
  ...
  findLastDisplayMessage(messages: MessageWrap[]) {
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (
        message.contentType === MessageContentTypeConst.time ||
  ...
  didMount(): void {
    ...
    this.messageListener = (message: Message) => {
      ...
      if (this.onMessageAppended) {
        this.onMessageAppended(message);
      }
    };
    ...
    this.cmdListener = (message: Message) => {
      ...
      } else if (cmdContent.cmd === "syncMessageExtra") {
        // 同步消息扩展
        if (this.isCMDForCurrentChannel(message, param)) {
          WKSDK.shared()
            .chatManager.syncMessageExtras(
              this.channel,
              this.findMaxExtraVersion()
            )
            .then((messageExtras) => {
              this.updateMessageByMessageExtras(messageExtras);
            });
        }
      } else if (cmdContent.cmd === "syncMessageReaction") {
        if (message.channel.isEqual(this.channel)) {
          WKApp.dataSource.channelDataSource
            .syncMessageReactions(this.channel)
            .then((reactions) => {
              this.updateMessageReactions(reactions);
            });
        }
      } else if (cmdContent.cmd === "syncPinnedMessage") {
        if (this.isCMDForCurrentChannel(message, param)) {
          ...
          if (this.messagesOfOrigin.length > 0) {
            this.browseToMessageSeq =
              this.messagesOfOrigin[
                this.messagesOfOrigin.length - 1
              ].messageSeq;
          }
          this.messagesOfOrigin = [];
          this.messages = [];
    ...
    if (this.channel.channelType === ChannelTypeGroup) {
      ...
      if (this.channelInfo) {
        ...
        WKSDK.shared()
          .channelManager.fetchChannelInfo(this.channel)
          .then(() => {
            this.channelInfo = WKSDK.shared().channelManager.getChannelInfo(
              this.channel
            );
    ...
    if (conversation) {
      ...
      if (unread > 0) {
        if (
          ...
          this.browseToMessageSeq =
            conversation.lastMessage.messageSeq - unread;
        }
      } else {
  ...
  async loadChannelInfoFinished() {
    ...
    if (this.channelInfo?.orgData?.group_type == SuperGroup) {
      ...
      WKSDK.shared().channelManager.subscribeCacheMap.set(
        this.channel.getChannelKey(),
        this.subscribers
      );
      WKSDK.shared().channelManager.notifySubscribeChangeListeners(
        this.channel
      );
      this.notifyListener();
    } else {
  ...
  async onDownArrow() {
    ...
    if (conversation && conversation.lastMessage) {
      if (this.messagesOfOrigin && this.messagesOfOrigin.length > 0) {
        const lastMessage =
          this.messagesOfOrigin[this.messagesOfOrigin.length - 1];
  ...
  // 重新加载订阅者
  reloadSubscribers() {
    this.subscribers = WKSDK.shared().channelManager.getSubscribes(
      this.channel
    );
    this.notifyListener();
  }
  ...
  updateMessageStatusBySendAck(ackPacket: SendackPacket) {
    ...
    if (message) {
      ...
      if (ackPacket.reasonCode === 1) {
        ...
        const sendingMessage = this.getSendingMessageWithClientMsgNo(
          message.clientMsgNo
        );
        if (sendingMessage) {
  ...
  isCMDForCurrentChannel(message: Message, param: any) {
    if (message.channel && message.channel.isEqual(this.channel)) {
      return true;
    }
    if (!param) {
      return false;
    ...
    return (
      ...
      Number(param.channel_type) === this.channel.channelType
    );
  }
  ...
  updateMessageReactions(reactions: any[]) {
    if (!reactions || reactions.length == 0) {
      return;
    }
    const reactionMap = new Map<string, any[]>();
    reactions.forEach((reaction) => {
      const messageID = `${reaction.message_id || reaction.messageID || ""}`;
      if (!messageID) {
        return;
      }
      ...
      current.push({
        ...
        isDeleted: reaction.is_deleted ?? reaction.isDeleted ?? 0,
        createdAt: reaction.created_at || reaction.createdAt,
      });
      reactionMap.set(messageID, current);
    });
    this.messages.forEach((message) => {
      const reactionsOfMessage = reactionMap.get(`${message.messageID}`);
      if (reactionsOfMessage) {
        (message.message as any).reactions = reactionsOfMessage;
      }
    });
    this.notifyListener();
  }

  syncPinnedMessages(version?: number) {
    WKApp.dataSource.channelDataSource
      .syncPinnedMessages(this.channel, version ?? this.pinnedVersion)
      .then((result) => {
        this.updatePinnedMessages(result);
      });
  }

  updatePinnedMessages(result: any) {
    if (!result) {
      return;
    }
    const pinnedMessages = result.pinned_messages || [];
    pinnedMessages.forEach((pinned: any) => {
      const messageID = `${pinned.message_id || ""}`;
      if (!messageID) {
        return;
      }
      this.pinnedMessageMap.set(messageID, pinned);
      if (pinned.version && pinned.version > this.pinnedVersion) {
        this.pinnedVersion = pinned.version;
      }
    });
    const messages = result.messages || [];
    messages.forEach((rawMessage: any) => {
      const message = Convert.toMessage(rawMessage);
      this.pinnedContentMap.set(`${message.messageID}`, message);
      const localMessage = this.findMessageWithMessageID(`${message.messageID}`);
      if (localMessage) {
        localMessage.message.remoteExtra = message.remoteExtra;
        (localMessage.message as any).reactions = (message as any).reactions;
      }
    });
    this.notifyListener();
  }

  currentPinnedMessage(): Message | undefined {
    const activePinnedMessages = Array.from(this.pinnedMessageMap.values())
      .filter((pinned) => Number(pinned.is_deleted) !== 1)
      .sort((a, b) => (b.version || 0) - (a.version || 0));
    for (const pinned of activePinnedMessages) {
      const message = this.pinnedContentMap.get(`${pinned.message_id}`);
      if (message) {
        return message;
      }
    }
  ...
  updateReplyMessageContent(extra: MessageExtra) {
    ...
    for (let i = this.messages.length - 1; i >= 0; i--) {
      ...
      if (
        message.content.reply.messageID &&
        message.content.reply.messageID === extra.messageID
      ) {
  ...
  findMessageWithMessageID(messageID: string): MessageWrap | undefined {
    ...
    for (let i = this.messages.length - 1; i >= 0; i--) {
      ...
      const currentMessageID = `${msg.messageIDStr || msg.message_idstr || msg.message_id_str || message.messageID || ""}`;
  ...
  appendMessage(messageWrap: MessageWrap) {
    ...
    this.messagesOfOrigin = this.limitOriginMessages(
      this.sortMessages(this.messagesOfOrigin),
      "latest"
    );
  ...
  refreshNewMsgCount() {
    ...
    if (this.browseToMessageSeq == 0) {
      ...
      // 没有给定最新的消息 没办法算未读数量
      this.unreadCount = 0;
    } else if (this.lastMessage.send) {
      // // 如果最后一条消息是自己发的 则新消息数量为0
      this.browseToMessageSeq = this.lastMessage.messageSeq;
      this.unreadCount = 0;
    } else if (this.lastMessage.messageSeq <= this.browseToMessageSeq) {
      // 如果最新消息的序号小于或等于预览到的 则最新消息为0
      this.unreadCount = 0;
    } else {
      if (this.lastMessage.messageSeq >= this.browseToMessageSeq) {
        this.unreadCount =
          this.lastMessage.messageSeq - this.browseToMessageSeq;
      }
    }
  ...
  // 获取第一屏消息
  requestMessagesOfFirstPage(
    lcateMessageSeq?: number,
    stateCallback?: () => void
  ) {
    ...
    if (lcateMessageSeq === undefined) {
      if (this.currentConversation) {
        ...
        if (this.currentConversation.unread > 0) {
          if (
            remoteExtra.keepMessageSeq != 0 &&
            remoteExtra.keepMessageSeq < this.browseToMessageSeq
          ) {
  ...
  async syncMessages(initMessageSeq?: number, stateCallback?: () => void) {
    ...
    const remoteMessages = await WKApp.conversationProvider.syncMessages(
      this.channel,
      opts
    );
  ...
  limitOriginMessages(
    ...
    keep: "latest" | "oldest" | "both" = "latest"
  ) {
    if (!messages || messages.length <= MAX_LOADED_ORIGIN_MESSAGES) {
      return messages;
    }
    if (keep === "oldest") {
      return messages.slice(0, MAX_LOADED_ORIGIN_MESSAGES);
    }
    if (keep === "both") {
      const headCount = Math.floor(MAX_LOADED_ORIGIN_MESSAGES / 2);
      const tailCount = MAX_LOADED_ORIGIN_MESSAGES - headCount;
      return [
        ...messages.slice(0, headCount),
        ...messages.slice(messages.length - tailCount),
      ];
    }
    return messages.slice(messages.length - MAX_LOADED_ORIGIN_MESSAGES);
  }

  // 刷新消息列表并定位到某条消息
  refreshAndLocateMessages(
    messages: MessageWrap[],
    locateMessage?: MessageWrap,
  ...
  refreshMessages(messages: MessageWrap[], callback?: () => void) {
    ...
    for (let i = 0; i < newMessages.length; i++) {
      ...
      if (message.contentType === MessageContentType.text) {
        message.content.text = ProhibitwordsService.shared.filter(
  ...
  async pulldownMessages() {
    ...
    if (minMessage?.messageSeq === 1) {
      // 如果最小messageSeq=1 说明下拉没消息了直接return
      return;
    }
    if (minMessage == null || minMessage.messageSeq <= 0) {
      // 没有消息直接return
      return;
    }
    ...
    let remoteMessages = await WKApp.conversationProvider.syncMessages(
      this.channel,
      opts
    );
    const newMessages = new Array<Message>();
    ...
    this.messagesOfOrigin = [
      ...this.toMessageWraps(newMessages),
      ...this.messagesOfOrigin,
    ];
    this.messagesOfOrigin = this.sortMessages(this.messagesOfOrigin);
    this.messagesOfOrigin = this.limitOriginMessages(this.messagesOfOrigin, "oldest");
    this.refreshAndLocateMessages(
      this.messagesOfOrigin,
      minMessage,
      false,
      () => {
  ...
  async pullupMessages() {
    ...
    if (maxMessage == null || maxMessage.messageSeq <= 0) {
      // 没有消息直接return
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        if (newMessages.length === 0) {
          ...
          if (
            ...
            this.formatMessageTime(preMessage) !==
              this.formatMessageTime(message)
          ) {
            const timeMessage = this.getTimeMessage(message.timestamp);
            newMessages.push(new MessageWrap(timeMessage));
          }
  ...
  distinctMessages(messages: Array<MessageWrap>) {
    ...
    for (let i = 0; i < messages.length; i++) {
      const message = messages[i];
      const key =
        message.clientMsgNo && message.clientMsgNo !== ""
          ? `client:${message.clientMsgNo}`
          : message.messageID
  ...
  async sendMessage(
    ...
    if (channel.channelType === ChannelTypePerson || channelInfo?.orgData.receipt === 1) {
