...
import { ListItem, ListItemIcon, ListItemSwitch } from "../ListItem";
...
import { getUserAutoTranslateOn, setUserAutoTranslateOn } from "../../Service/Translate";
    ...
    sections(context: RouteContext<any>) {
        ...
        sections.push(new Section({
            rows: [
                ...
                ...(WKApp.remoteConfig.autoTranslateOn ? [
                    new Row({
                        cell: ListItemSwitch,
                        properties: {
                            title: "自动翻译",
                            checked: getUserAutoTranslateOn(WKApp.loginInfo.uid),
                            onCheck: (checked: boolean) => {
                                setUserAutoTranslateOn(WKApp.loginInfo.uid, checked)
                                Toast.success(checked ? "已开启自动翻译" : "已关闭自动翻译")
                                this.notifyListener()
                            }
                        }
                    })
