...
export interface MessageInputContext {
    ...
    send(): void
    clearText(): void
    registerSendInterceptor(key: string, handler: (text: string, mention?: MentionModel) => boolean): void
    unregisterSendInterceptor(key: string): void
}
...
export default class MessageInput extends Component<MessageInputProps, MessageInputState> implements MessageInputContext {
    ...
    inputRef: any
    eventListener: any
    sendInterceptors: Map<string, (text: string, mention?: MentionModel) => boolean>
    constructor(props: MessageInputProps) {
        super(props)
        this.toolbars = []
        this.sendInterceptors = new Map()
        this.state = {
            value: "",
            mentionCache: {},
    ...
    send() {
        ...
        let formatValue = value ? this.formatMentionText(value) : "";
        let mention = this.parseMention(formatValue)
        if (this.sendInterceptors.size > 0) {
            for (const handler of this.sendInterceptors.values()) {
                if (handler(formatValue, mention)) {
                    this.clearText()
                    return
                }
            }
        }
        if (this.props.onSend && formatValue && formatValue.trim() !== "") {
    ...
    clearText() {
        ...
        this.props.context.clearDraft()
    }

    registerSendInterceptor(key: string, handler: (text: string, mention?: MentionModel) => boolean) {
        this.sendInterceptors.set(key, handler)
    }

    unregisterSendInterceptor(key: string) {
        this.sendInterceptors.delete(key)
    }
