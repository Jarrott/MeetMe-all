    ...
    render() {
        ...
        return <MessageBase message={message} context={context} hiddeBubble={true}>
            <div style={{ width: scaleSize.width, height: scaleSize.height }}>
                <LazyMediaImage alt="" src={WKApp.dataSource.commonDataSource.getImageURL(content.url, { width: Math.ceil(scaleSize.width), height: Math.ceil(scaleSize.height) })} style={{ borderRadius:'5px',width: scaleSize.width, height: scaleSize.height, objectFit: "cover" }} />
            </div>
        </MessageBase>
    }
}
