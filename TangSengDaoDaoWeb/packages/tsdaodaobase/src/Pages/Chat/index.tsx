...
> {
  ...
  private channelSearchMouseDownListener = (event: MouseEvent) => {
    if (!this.state.showChannelSearch) {
      return;
    }
    const target = event.target as HTMLElement;
    if (
      target.closest(".wk-chat-header-inline-search") ||
      target.closest(".wk-chat-conversation-header-search")
    ) {
      return;
    }
    this.setState({
  ...
  componentDidMount() {
    ...
    document.addEventListener("mousedown", this.channelSearchMouseDownListener);
  }

  componentWillUnmount() {
    WKSDK.shared().channelManager.removeListener(this.channelInfoListener);
    document.removeEventListener("mousedown", this.channelSearchMouseDownListener);
  }
  ...
  render(): React.ReactNode {
    ...
    const { showChannelSetting, showChannelSearch } = this.state;
    ...
    return (
      ...
      >
          ...
          >
            <div className="wk-chat-conversation-header-content">
              ...
              <div className="wk-chat-conversation-header-right">
                ...
                <div
                  className="wk-chat-conversation-header-right-item wk-chat-conversation-header-search"
                  title="搜索聊天记录"
                  onClick={(e) => {
                    e.stopPropagation();
                    this.setState({
        ...
        {showChannelSearch ? (
          <div
            className="wk-chat-header-inline-search"
            onClick={(event) => {
...
export default class ChatPage extends Component<any> {
  vm!: ChatVM;
  private globalSearchMouseDownListener = (event: MouseEvent) => {
    if (!this.vm?.showGlobalSearch) {
      return;
    }
    const target = event.target as HTMLElement;
    if (
      target.closest(".wk-chat-left-inline-search") ||
      target.closest(".wk-chat-global-search-trigger")
    ) {
      return;
    }
    this.vm.showGlobalSearch = false;
  };
  constructor(props: any) {
    super(props);
  }
  ...
  componentDidMount() {
    ...
    document.addEventListener("mousedown", this.globalSearchMouseDownListener);
  }

  componentWillUnmount() {
    document.removeEventListener("mousedown", this.globalSearchMouseDownListener);
  }
              ...
              >
                <div className="wk-chat-content-left">
                  <div className="wk-chat-search">
                    ...
                    <div
                      className="wk-chat-global-search-trigger"
                    ...
                    {vm.showGlobalSearch ? (
                      <div
                        className="wk-chat-left-inline-search"
                        onClick={(event) => {
                          event.stopPropagation();
                        }}
                      >
