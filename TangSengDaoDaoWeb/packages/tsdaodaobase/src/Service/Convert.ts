...
import { UnsupportContent } from "../Messages/Unsupport";
import { MessageContentTypeConst } from "./Const";
import { resolveStaticAssetURL } from "../Utils/staticAsset";
...
export class Convert {
  static toConversation(conversationMap: any): Conversation {
    ...
    if (conversationMap["extra"]) {
      conversation.remoteExtra = this.toConversationExtra(
        conversation.channel,
        conversationMap["extra"]
      );
    }
  ...
  static toReminder(reminderMap: any): Reminder {
    ...
    reminder.channel = new Channel(
      reminderMap["channel_id"],
      reminderMap["channel_type"]
    );
  ...
  static toConversationExtra(
    channel: Channel,
    conversationExtraMap: any
  ): ConversationExtra {
  ...
  static toMessage(msgMap: any): Message {
    ...
    (message as any).messageIDStr = message.messageID;
    if (msgMap["header"]) {
    ...
    if (msgMap["reactions"]) {
      (message as any).reactions = msgMap["reactions"].map((reaction: any) => {
        return {
          uid: reaction.uid,
          name: reaction.name,
          seq: reaction.seq,
    ...
    let contentObj = msgMap["payload"];
    if (typeof contentObj === "string") {
      try {
        contentObj = JSON.parse(contentObj);
      } catch (error) {
        console.warn("消息 payload 不是有效 JSON", msgMap, error);
        contentObj = undefined;
      }
    }
    let contentType = 0;
    if (contentObj) {
      contentType = Number(contentObj.type || 0);
    }
    let messageContent = WKSDK.shared().getMessageContent(contentType);
    if (
      contentType === MessageContentTypeConst.file &&
      messageContent.contentType !== MessageContentTypeConst.file
    ) {
    ...
    if (contentObj) {
      try {
        ...
        console.warn("消息内容解析失败，已降级显示", {
          message_id: msgMap["message_idstr"] || msgMap["message_id"],
          message_seq: msgMap["message_seq"],
          content_type: contentType,
          payload: contentObj,
          error,
        });
        messageContent = new UnsupportContent("[消息解析失败，请在手机端查看]");
      }
    }
  ...
  static userToChannelInfo(data: any): ChannelInfo {
    ...
    if (data.category === "system" || data.category === "customerService") {
      // 官方账号
      channelInfo.orgData.identityIcon = resolveStaticAssetURL("/identity_icon/official.png?v=20260628-icon");
      channelInfo.orgData.identitySize = { width: "18px", height: "18px" };
    } else if (data.category === "visitor") {
      channelInfo.orgData.identityIcon = resolveStaticAssetURL("/identity_icon/visitor.png?v=20260628-icon");
  ...
  static groupToChannelInfo(data: any): ChannelInfo {
    ...
    channelInfo.orgData.allowMemberPinnedMessage =
      data.allow_member_pinned_message ??
      data.extra?.allow_member_pinned_message;
