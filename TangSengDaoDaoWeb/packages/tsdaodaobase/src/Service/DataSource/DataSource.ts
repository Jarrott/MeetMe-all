...
export class DataSource {
  ...
  public notifyContactsChange() {
    if (this.contactsChangeListeners) {
      this.contactsChangeListeners.forEach(
        (listener: ContactsChangeListener) => {
          if (listener) {
            listener();
          }
        }
...
export interface ICommonDataSource {
  ...
  getStickerUploadURL(ext: string): Promise<string>;
  uploadStickerFile(file: File): Promise<string>;
  addUserSticker(req: {
    category: string;
    path: string;
    placeholder?: string;
  ...
  translateMessage(
    ...
    targetLang: string,
    sourceLang?: string
  ): Promise<MessageTranslationResp>;
}

export interface MessageTranslationResp {
  message_id: string;
  source_lang: string;
  target_lang: string;
...
export interface IChannelDataSource {
   ...
   * @param channel 群频道
   * @param uid 成员UID
   * @param action 0 解禁，1 禁言
   * @param key 禁言时长：1=1分钟，2=10分钟，3=1小时，4=1天，5=1周，6=1个月
   */
  forbiddenSubscriber(
    channel: Channel,
    uid: string,
   ...
   * @param channel
   */
  disbandChannel(channel: Channel): Promise<void>;
   ...
   * @param version 同步版本
   */
  syncPinnedMessages(channel: Channel, version?: number): Promise<any>;
