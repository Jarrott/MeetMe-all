...
export class NotificationUtil {
  ...
  private permissionRequestRegistered: boolean = false;
  ...
  public isNotificationSupported(): boolean {
    return this.isElectronNativeNotificationAvailable() || !!(window.Notification && Notification.permission !== "denied");
  }
  ...
  /**
   * Browsers are much more reliable about showing the permission prompt after a user gesture.
   */
  public registerPermissionRequestOnUserGesture(): void {
    if (!window.Notification || Notification.permission !== "default" || this.permissionRequestRegistered) {
      return;
    }

    this.permissionRequestRegistered = true;
    const request = () => {
      this.requestPermission().finally(() => {
  ...
  private isElectronNativeNotificationAvailable(): boolean {
    ...
    const isElectronRuntime =
      !!window.__POWERED_ELECTRON__ ||
      window.location.protocol === "file:" ||
      /Electron/i.test(window.navigator?.userAgent || "");
    return !!(isElectronRuntime && window.electronNotification);
  }
  ...
  private async createNotification(options: NotificationOptions): Promise<Notification | null> {
    ...
    // Desktop packages should use Electron native notifications directly. The hosted
    // web Notification permission can be default/denied and should not block native IPC.
