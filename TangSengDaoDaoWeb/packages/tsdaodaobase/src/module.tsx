...
import { ImageGroupCell, ImageGroupContent } from "./Messages/ImageGroup";
...
import RTCToolbarButton from "./Components/RTCToolbarButton";
...
import { getMessagePlainText, getPreferredTargetLang, shouldSkipAutoTranslate } from "./Service/Translate";
import { RTCContent, RTCCell } from "./Messages/RTC";
import RTCService from "./Service/RTC";

const rtcMessageTypes = [
  MessageContentTypeConst.rtcP2P,
  MessageContentTypeConst.rtcResult,
  MessageContentTypeConst.rtcSwitchToVideo,
  MessageContentTypeConst.rtcSwitchToVideoReply,
  MessageContentTypeConst.rtcCancel,
  MessageContentTypeConst.rtcSwitchToAudio,
  MessageContentTypeConst.rtcData,
  MessageContentTypeConst.rtcMissed,
  MessageContentTypeConst.rtcReceived,
  MessageContentTypeConst.rtcRefue,
  MessageContentTypeConst.rtcAccept,
  MessageContentTypeConst.rtcHangup,
];

const exactMessageID = (message: Message): string => {
...
export default class BaseModule implements IModule {
  ...
  init(): void {
    ...
    notificationUtil.registerPermissionRequestOnUserGesture();
    ...
    WKSDK.shared().register(MessageContentTypeConst.imageGroup, () => new ImageGroupContent()); // 多图组
    ...
    rtcMessageTypes.forEach((contentType) => {
      WKSDK.shared().register(
        contentType,
        (registeredType?: number) => new RTCContent(undefined, undefined, registeredType || contentType)
      );
    });
    ...
    WKSDK.shared().chatManager.addCMDListener((message: Message) => {
      ...
      if (cmdContent.cmd?.startsWith("rtc.p2p.")) {
        RTCService.shared.handleSignalMessage(message);
        return;
      }
  ...
  registerChatToolbars() {
    ...
    WKApp.endpoints.registerChatToolbar("chattoolbar.audioCall", (ctx) => {
      const channel = ctx.channel();
      if (channel.channelType !== ChannelTypePerson) {
        return undefined;
      }
    ...
    WKApp.endpoints.registerChatToolbar("chattoolbar.videoCall", (ctx) => {
      const channel = ctx.channel();
      if (channel.channelType !== ChannelTypePerson) {
        return undefined;
      }
  ...
  private likedByMe(message: Message, emoji: string) {
    const reactions = ((message as any).reactions || []) as any[];
    return reactions.some((reaction) => {
      return (
        reaction.uid === WKApp.loginInfo.uid &&
  ...
  private applyLocalReaction(message: Message, emoji: string, isCancel: boolean) {
    const loginUID = WKApp.loginInfo.uid || "";
    const reactions = (((message as any).reactions || []) as any[]).filter(
      (reaction) => !(reaction.uid === loginUID && reaction.emoji === emoji)
    );
    if (!isCancel) {
      reactions.push({
        uid: loginUID,
      ...
      (message as any).reactionEffectAt = Date.now();
    } else {
      (message as any).reactionEffectAt = 0;
    }
  ...
  private loadImageAsPng(url: string) {
  ...
  private htmlEscape(text: string) {
  ...
  private appendFilename(url: string, filename: string) {
    if (!url) {
      return url;
    }
    return `${url}${url.indexOf("?") === -1 ? "?" : "&"}filename=${encodeURIComponent(filename)}`;
  }

  private imageURLForCopy(path?: string, imgData?: string) {
    if (path) {
      return this.appendFilename(WKApp.dataSource.commonDataSource.getImageURL(path), "image.png");
    }
    return imgData || "";
  }

  private fileURLForCopy(path?: string) {
    if (!path) {
      return "";
    }
    return this.appendFilename(WKApp.dataSource.commonDataSource.getFileURL(path), "video.mp4");
  }

  private mediaCaption(message: Message) {
    const content = (message.content || {}) as any;
    return `${content.caption || content.text || content.content || ""}`.trim();
  }

  private mediaCopyData(message: Message) {
    const content = (message.content || {}) as any;
    const caption = this.mediaCaption(message);
    if (message.contentType === MessageContentType.image) {
      const imageURL = this.imageURLForCopy(content.url || content.remoteUrl, content.imgData);
      return {
        kind: "image",
        title: caption ? "复制图文" : "复制图片",
        mediaURL: imageURL,
        plainText: caption || imageURL,
        html: `${imageURL ? `<img src="${this.htmlEscape(imageURL)}" />` : ""}${caption ? `<div>${this.htmlEscape(caption).replace(/\n/g, "<br/>")}</div>` : ""}`,
      };
    }
    if (message.contentType === MessageContentTypeConst.imageGroup) {
      const images = Array.isArray(content.images) ? content.images : [];
      const urls = images.map((item: any) => this.imageURLForCopy(item.url, item.imgData)).filter(Boolean);
      const firstURL = urls[0] || "";
      const linksText = urls.length > 0 ? urls.join("\n") : "";
      return {
        kind: "image",
        title: caption ? "复制图文" : "复制图片",
        mediaURL: firstURL,
        plainText: [caption, linksText].filter(Boolean).join("\n"),
        html: `${urls.map((url: string) => `<img src="${this.htmlEscape(url)}" />`).join("<br/>")}${caption ? `<div>${this.htmlEscape(caption).replace(/\n/g, "<br/>")}</div>` : ""}`,
      };
    }
    if (message.contentType === MessageContentTypeConst.smallVideo) {
      const videoURL = this.fileURLForCopy(content.url || content.remoteUrl);
      return {
        kind: "video",
        title: caption ? "复制图文" : "复制视频链接",
        mediaURL: "",
        plainText: [caption, videoURL].filter(Boolean).join("\n"),
        html: `${videoURL ? `<a href="${this.htmlEscape(videoURL)}">${this.htmlEscape(videoURL)}</a>` : ""}${caption ? `<div>${this.htmlEscape(caption).replace(/\n/g, "<br/>")}</div>` : ""}`,
      };
    }
    return undefined;
  }

  private async copyMediaMessage(message: Message) {
    const data = this.mediaCopyData(message);
    if (!data) {
      return;
    }
    const plainText = data.plainText || data.mediaURL || "";
    try {
      if (data.mediaURL && navigator.clipboard && (window as any).ClipboardItem) {
        const imageBlob = await this.loadImageAsPng(data.mediaURL);
        ...
        if (data.html) {
          clipboardData["text/html"] = new Blob([data.html], { type: "text/html" });
        }
        await navigator.clipboard.write([new (window as any).ClipboardItem(clipboardData)]);
        Toast.success(data.title === "复制图文" ? "图文已复制" : "媒体已复制");
        return;
      }
      await this.copyText(plainText);
      Toast.success(data.title === "复制图文" ? "图文文字已复制" : "链接已复制");
    } catch (err) {
      try {
        await this.copyText(plainText);
        Toast.success("当前浏览器不支持直接复制媒体，已复制文字和链接");
      } catch (copyErr) {
  ...
  registerMessageContextMenus() {
    ...
    WKApp.endpoints.registerMessageContextMenus(
      ...
      (message) => {
        if (![MessageContentType.image, MessageContentTypeConst.imageGroup, MessageContentTypeConst.smallVideo].includes(message.contentType)) {
          return null;
        }
        const copyData = this.mediaCopyData(message);

        return {
          title: copyData?.title || "复制",
          onClick: async () => {
            await this.copyMediaMessage(message);
          },
        };
      },
      1100
    ...
    WKApp.endpoints.registerMessageContextMenus(
      ...
      (message, context) => {
        if (!WKApp.remoteConfig.autoTranslateOn || message.contentType !== MessageContentType.text) {
          return null;
        }
        ...
        return {
          title: (message as any).translationText ? "重新翻译" : "翻译",
          onClick: () => {
            (message as any).translationLoading = true;
            context.refresh();
            WKApp.dataSource.commonDataSource
              .translateMessage(message, targetLang)
              .then((resp) => {
                (message as any).translationText = resp.translated_text;
                (message as any).translationTargetLang = resp.target_lang;
                (message as any).translationLoading = false;
                context.refresh();
              })
              .catch((err) => {
                (message as any).translationLoading = false;
                context.refresh();
                Toast.error(err.msg || "翻译失败");
              });
          },
        };
      },
    ...
    WKApp.endpoints.registerMessageContextMenus(
      "contextmenus.reaction.like",
      (message, context) => {
        if (exactMessageID(message) === "") {
          return null;
        }
        const liked = this.likedByMe(message, "👍");
        return {
          title: liked ? "取消点赞" : "点赞",
          onClick: () => {
            WKApp.dataSource.channelDataSource
              ...
              .then(() => {
                this.applyLocalReaction(message, "👍", liked);
                context.refresh();
                Toast.success(liked ? "已取消点赞" : "已点赞");
              })
              .catch((err) => {
                Toast.error(err.msg);
              });
    ...
    WKApp.endpoints.registerMessageContextMenus(
      ...
      (message, context) => {
        ...
        const isPinned = (message.remoteExtra as any)?.isPinned === true;
        return {
          ...
          onClick: () => {
            WKApp.dataSource.channelDataSource
              ...
              .then(() => {
                (message.remoteExtra as any).isPinned = !isPinned;
                context.syncPinnedMessages();
                context.refresh();
    ...
    WKApp.endpoints.registerMessageContextMenus(
      "contextmenus.edit",
      (message, context) => {
        if (!message.send || exactMessageID(message) === "" || !WKApp.shared.supportEdit.includes(message.contentType)) {
          return null;
        }
        return {
          title: "编辑",
    ...
    WKApp.endpoints.registerMessageContextMenus(
      "contextmenus.revoke",
      (message, context) => {
        if (exactMessageID(message) === "" || !message.clientMsgNo) {
          return null;
        }
  ...
  registerUserInfo() {
    ...
    WKApp.shared.userInfoRegister(
      "userinfo.group.manage",
      (context: RouteContext<UserInfoRouteData>) => {
        const data = context.routeData();
        const fromChannel = data.fromChannel;
        const targetSubscriber = data.fromSubscriberOfUser;
        const mySubscriber = data.subscriberOfMy;
        if (
          data.isSelf ||
          !fromChannel ||
          fromChannel.channelType !== ChannelTypeGroup ||
          !targetSubscriber ||
          !mySubscriber
        ) {
          return;
        ...
        const myRole = mySubscriber.role;
        const targetRole = targetSubscriber.role;
        ...
        if (isOwner && targetRole !== GroupRole.owner) {
          rows.push(
            new Row({
              cell: ListItem,
              properties: {
                ...
                onClick: () => {
                  const action =
                    targetRole === GroupRole.manager
                      ? WKApp.dataSource.channelDataSource.managerRemove(
                          fromChannel,
                          [data.uid]
                        )
                      : WKApp.dataSource.channelDataSource.managerAdd(
                          fromChannel,
                          [data.uid]
                        );
                  action
                    .then(() => {
                      Toast.success("已更新管理员");
                      data.refresh();
                    })
                    .catch((err) => {
                      Toast.error(err.msg);
        ...
        if (canManageTarget) {
          const forbiddenExpirTime =
            targetSubscriber.orgData?.forbidden_expir_time || 0;
          const forbiddenActive =
            forbiddenExpirTime > Math.floor(new Date().getTime() / 1000);
          rows.push(
            new Row({
              cell: ListItem,
              properties: {
                title: forbiddenActive ? "解除禁言" : "禁言1小时",
                onClick: () => {
                  WKApp.dataSource.channelDataSource
                    .forbiddenSubscriber(
                      fromChannel,
                    ...
                    .then(() => {
                      Toast.success(forbiddenActive ? "已解除禁言" : "已禁言");
                      data.refresh();
                    })
                    .catch((err) => {
                      Toast.error(err.msg);
  ...
  registerChannelSettings() {
    ...
    WKApp.shared.channelSettingRegister(
      ...
      (context) => {
        ...
        let inviteSelectItems: IndexTableItem[] = [];
        let inviteFinishButtonContext: FinishButtonContext;
        let managerSelectItems: Subscriber[] = [];
        let managerFinishButtonContext: FinishButtonContext;
        const disableSelectList = data.subscribers.map((subscriber) => {
          return subscriber.uid;
        });
        const disableManagerSelectList = data.subscribers
          .filter((subscriber) => {
            return (
        ...
        return new Section({
          rows: [
            new Row({
              ...
              properties: {
                title: "邀请成员入群",
                subTitle: "发起邀请，由群主或管理员确认",
                onClick: () => {
                  context.push(
                    <ContactsSelect
                      onSelect={(items) => {
                        inviteSelectItems = items;
                        inviteFinishButtonContext.disable(items.length === 0);
                      }}
                      disableSelectList={disableSelectList}
                    ></ContactsSelect>,
                    {
                      ...
                      showFinishButton: true,
                      onFinish: async () => {
                        inviteFinishButtonContext.loading(true);
                        await WKApp.dataSource.channelDataSource
                          .inviteSubscribers(
                            channel,
                            inviteSelectItems.map((item) => {
                              return item.id;
                            }),
                            ""
                          )
                        ...
                        inviteFinishButtonContext.loading(false);
                      },
                      onFinishContext: (ctx) => {
                        inviteFinishButtonContext = ctx;
                        inviteFinishButtonContext.disable(true);
                      },
                    }
                  );
                },
            ...
            new Row({
              ...
              properties: {
                title: "设置群管理员",
                subTitle: "选择成员协助群主管理群聊",
                onClick: () => {
                  if (data.subscriberOfMe?.role !== GroupRole.owner) {
                    Toast.warning("只有群主才能设置管理员");
                    return;
                  }
                  context.push(
                    <SubscriberList
                      channel={channel}
                      canSelect={true}
                      disableSelectList={disableManagerSelectList}
                      onSelect={(items) => {
                        managerSelectItems = items;
                        managerFinishButtonContext.disable(items.length === 0);
                      }}
                    />,
                    {
                      title: "设置群管理员",
                      showFinishButton: true,
                      onFinish: async () => {
                        managerFinishButtonContext.loading(true);
                        await WKApp.dataSource.channelDataSource
                          .managerAdd(
                            channel,
                            managerSelectItems.map((item) => {
                              return item.uid;
                            })
                          )
                          .then(() => {
                            Toast.success("已设置管理员");
                            data.refresh();
                            context.pop();
                          })
                          .catch((err) => {
                        ...
                        managerFinishButtonContext.loading(false);
                      },
                      onFinishContext: (ctx) => {
                        managerFinishButtonContext = ctx;
                        managerFinishButtonContext.disable(true);
                      },
                    }
                  );
                },
            ...
            new Row({
              ...
              properties: {
                ...
                onCheck: (v: boolean, ctx: ListItemSwitchContext) => {
                  if (!data.isManagerOrCreatorOfMe) {
                    Toast.warning("只有群主或管理员才能设置全员禁言");
                    return;
                  }
                  ctx.loading = true;
                  ChannelSettingManager.shared
            ...
            new Row({
              ...
              properties: {
                ...
                onCheck: (v: boolean, ctx: ListItemSwitchContext) => {
                  if (!data.isManagerOrCreatorOfMe) {
                    Toast.warning("只有群主或管理员才能设置邀请验证");
                    return;
                  }
                  ctx.loading = true;
                  ChannelSettingManager.shared
            ...
            new Row({
              ...
              properties: {
                title: "允许成员置顶消息",
                checked: channelInfo?.orgData?.allowMemberPinnedMessage === 1,
                onCheck: (v: boolean, ctx: ListItemSwitchContext) => {
                  if (!data.isManagerOrCreatorOfMe) {
                    Toast.warning("只有群主或管理员才能设置置顶权限");
                    return;
                  }
                  ctx.loading = true;
                  ChannelSettingManager.shared
                    .allowMemberPinnedMessage(v, channel)
                    .then(() => {
                      ctx.loading = false;
                      data.refresh();
                    })
            ...
            new Row({
              ...
              properties: {
                ...
                onClick: () => {
                  if (!data.isManagerOrCreatorOfMe) {
                    Toast.warning("只有群主或管理员才能清空置顶消息");
                    return;
                  }
                  WKApp.shared.baseContext.showAlert({
                    content: "是否清空当前群聊的所有置顶消息？",
                    onOk: async () => {
                      await WKApp.dataSource.channelDataSource
                        .clearPinnedMessages(channel)
                        .then(() => {
                          Toast.success("已清空置顶消息");
                        })
                        .catch((err) => {
                          Toast.error(err.msg);
                        });
                ...
                onClick: () => {
                  ...
                  WKApp.shared.baseContext.showAlert({
                    ...
                    onOk: async () => {
                      try {
                        if (isOwner) {
                          await WKApp.dataSource.channelDataSource.disbandChannel(
                            data.channel
                          );
                        } else {
