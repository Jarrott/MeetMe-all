...
export class ConversationProvider implements IConversationProvider {
    ...
    revokeMessage(message: Message): Promise<void> {
        return WKApp.apiClient.post(`message/revoke?channel_id=${message.channel.channelID}&channel_type=${message.channel.channelType}&message_id=${exactMessageID(message)}&client_msg_no=${message.clientMsgNo}`)
    }
    ...
    async syncGetMessages(channel: Channel, opts: SyncMessageOptions): Promise<Message[]> {
        ...
        var resp = await WKApp.apiClient.post(`message/channel/sync`, { "limit": limit, "channel_id": channel.channelID, "channel_type": channel.channelType, "start_message_seq": opts.startMessageSeq || 0, "end_message_seq": opts.endMessageSeq || 0, "pull_mode": opts.pullMode, "device_uuid": WKApp.shared.deviceId || "" });
        ...
        if (messageList) {
            for (const msg of messageList) {
                try {
                    ...
                    console.warn("跳过解析失败的消息", msg, error);
                }
            }
