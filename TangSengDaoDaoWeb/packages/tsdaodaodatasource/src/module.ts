import { Convert, GroupRole, IModule, WKApp, resolveStaticAssetURL } from "@tsdaodao/base";
import {
  Channel,
  ChannelInfo,
  ChannelTypeGroup,
...
export default class DataSourceModule implements IModule {
      ...
      if (messages && messages.length > 0) {
        for (const message of messages) {
          ...
          if (message.messageSeq && message.messageSeq > 0) {
            messageSeqs.push(message.messageSeq);
          }
        }
      }
      if (messageIDs.length === 0 && messageSeqs.length === 0) {
        return;
      }
  ...
  setSyncConversationsCallback() {
    WKSDK.shared().config.provider.syncConversationsCallback = async (
      ...
      resp = await WKApp.apiClient.post("conversation/sync", {
        version: syncFilter.version || 0,
        device_uuid: WKApp.shared.deviceId || "",
        last_msg_seqs: syncFilter.lastMsgSeqs || syncFilter.last_msg_seqs || "",
        msg_count: syncFilter.msgCount || syncFilter.msg_count || 1,
      });
      if (resp) {
