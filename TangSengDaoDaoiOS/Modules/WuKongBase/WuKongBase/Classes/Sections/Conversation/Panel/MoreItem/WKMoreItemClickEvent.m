...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    ...
    if(!fileURL || !self.gloabContext) {
        return;
    }
    WKFileContent *content = [WKFileContent fileWithURL:fileURL];
    if(!content.sourceLocalPath || ![[NSFileManager defaultManager] fileExistsAtPath:content.sourceLocalPath]) {
        [[WKNavigationManager shared].topViewController.view showMsg:LLang(@"文件读取失败")];
        return;
    }
    [self.gloabContext sendMessage:content];
