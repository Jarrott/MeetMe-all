...
static NSString *const WKLocalStickerCategory = @"local";
static NSString *const WKLocalStickerPathPrefix = @"/stickers/local/";
...
-(void) requestHotGif {
    ...
    if([keyword isEqualToString:WKLocalStickerCategory]) {
        self.stickerPackage = [self localStickerPackage];
        cacheGifDict[keyword] = self.stickerPackage;
        [self.collectionView reloadData];
        return;
    }
...
-(WKStickerPackage*) localStickerPackage {
    WKStickerPackage *package = [WKStickerPackage new];
    package.title = @"MeetMe";
    package.category = WKLocalStickerCategory;
    package.cover = [WKLocalStickerPathPrefix stringByAppendingString:@"great.svg"];
    package.added = true;
    NSMutableArray<WKSticker*> *stickers = [NSMutableArray array];
    NSArray<NSString*> *names = @[@"received", @"wait", @"thanks", @"busy", @"go", @"done", @"read", @"great", @"ok", @"nice", @"onway", @"celebrate", @"sorry", @"callme", @"coffee", @"question"];
    for (NSString *name in names) {
        WKSticker *sticker = [WKSticker new];
        sticker.category = WKLocalStickerCategory;
        sticker.format = @"image";
        sticker.path = [WKLocalStickerPathPrefix stringByAppendingFormat:@"%@.svg", name];
        sticker.placeholder = [self localStickerSvg:name];
        sticker.width = @(160);
        sticker.height = @(160);
...
-(NSString*) localStickerSvg:(NSString*)name {
    NSString *resourcePath = [NSString stringWithFormat:@"Other/Stickers/Local/%@.svg", name];
    NSBundle *bundle = [[WKApp shared] resourceBundle:@"WuKongBase_resources"];
    NSString *filePath = [bundle pathForResource:resourcePath ofType:nil];
    if(filePath.length == 0) {
        return @"";
    }
    NSString *svg = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
    return svg?:@"";
}
