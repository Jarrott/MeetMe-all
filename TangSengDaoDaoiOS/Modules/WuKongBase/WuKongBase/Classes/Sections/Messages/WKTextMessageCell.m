...
#define splitWidth 3.0f
#define replyNameLeftSpace 8.0f
#define replyDividerHeight 1.0f
#define replyDividerTopSpace 7.0f
...
@property(nonatomic,strong) UIView *replyDividerView;
...
+ (CGSize)contentSizeForMessage:(WKMessageModel *)model {
    ...
    if([self hasReply:model]) {
        ...
        CGFloat replyHeight = replyNameSize.height + 2.0f + replyContentSize.height + replyDividerTopSpace + replyDividerHeight;
        CGFloat replyHeaderWidth = replyNameSize.width + replyNameLeftSpace + replyAvatarSize + splitWidth;
        CGFloat replyContentWidth = replyContentSize.width + splitWidth;
        size = CGSizeMake(MAX(MAX(messageTextSize.width, replyHeaderWidth), replyContentWidth) , messageTextSize.height + replyHeight + textTopSpace + nameTopSpace);
    }
...
//    [self.textLbl setTextColor:[WKApp shared].config.defaultTextColor];
    ...
    [self.replyBox addSubview:self.replyDividerView];
...
//    NSString *text = textContent.content;
    ...
    if([self.messageModel isSend]) {
        UIColor *replyTextColor = [UIColor colorWithWhite:1.0f alpha:0.86f];
        self.replyContentLbl.textColor = replyTextColor;
        self.replyNameLbl.textColor = replyTextColor;
        self.splitView.backgroundColor = [UIColor colorWithWhite:1.0f alpha:0.45f];
        self.replyDividerView.backgroundColor = [UIColor colorWithWhite:1.0f alpha:0.28f];
    }else{
        self.replyContentLbl.textColor =[WKApp shared].config.tipColor;
        self.replyNameLbl.textColor = [WKApp shared].config.tipColor;
        self.splitView.backgroundColor = [WKApp shared].config.themeColor;
        self.replyDividerView.backgroundColor = [WKApp shared].config.lineColor;
    }
...
-(void) layoutSubviews {
    ...
    if([[self class] hasReply:self.messageModel]) {
        ...
        self.replyBox.lim_height = replyNameSize.height + 2.0f + replyContentSize.height + replyDividerTopSpace + replyDividerHeight;
        
        self.splitView.lim_left = 0.0f;
        self.splitView.lim_top = 0.0f;
        self.splitView.lim_height = replyNameSize.height + 2.0f + replyContentSize.height;
        self.splitView.lim_width = splitWidth;
        
        self.replyAvatarIcon.lim_left = self.splitView.lim_right + 5.0f;
        ...
        self.replyDividerView.lim_left = 0.0f;
        self.replyDividerView.lim_top = self.replyContentLbl.lim_bottom + replyDividerTopSpace;
        self.replyDividerView.lim_width = self.replyBox.lim_width;
        self.replyDividerView.lim_height = replyDividerHeight;
...
- (UIView *)splitView {
    if(!_splitView) {
        ...
        _splitView.layer.cornerRadius = splitWidth/2.0f;
        _splitView.layer.masksToBounds = YES;
    }
    return _splitView;
}

- (UIView *)replyDividerView {
    if(!_replyDividerView) {
        _replyDividerView = [[UIView alloc] init];
        _replyDividerView.backgroundColor = [WKApp shared].config.lineColor;
    }
    return _replyDividerView;
}
