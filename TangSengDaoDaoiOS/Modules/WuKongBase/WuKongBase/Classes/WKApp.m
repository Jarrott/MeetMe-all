...
-(NSURL*) getFileFullUrl:(NSString*)path{
    ...
    NSString *localStickerPrefix = @"/stickers/local/";
    NSString *normalizedPath = path;
    if([normalizedPath hasPrefix:@"/"]) {
        normalizedPath = [normalizedPath substringFromIndex:1];
    }
    if([path hasPrefix:localStickerPrefix] || [normalizedPath hasPrefix:@"stickers/local/"]) {
        NSString *fileName = [normalizedPath lastPathComponent];
        NSString *resourcePath = [NSString stringWithFormat:@"Other/Stickers/Local/%@", fileName];
        NSBundle *bundle = [self resourceBundle:@"WuKongBase_resources"];
        NSString *filePath = [bundle pathForResource:resourcePath ofType:nil];
    ...
    if([path hasPrefix:@"bundle://"]) {
        ...
        NSString *bundleName = components.host;
        NSString *resourcePath = components.path;
        if([resourcePath hasPrefix:@"/"]) {
            resourcePath = [resourcePath substringFromIndex:1];
        }
        if(bundleName.length > 0 && resourcePath.length > 0) {
            NSBundle *bundle = [self resourceBundle:bundleName];
            NSString *filePath = [bundle pathForResource:resourcePath ofType:nil];
            if(filePath.length > 0) {
                return [NSURL fileURLWithPath:filePath];
            }
...
-(void) initPointMethods {
    ...
    // 本地贴纸
    [self setMethod:@"meetme.local.stickers" handler:^id _Nullable(id  _Nonnull param) {
        return [[WKStickerGIFContentView alloc] initWithKeyword:@"local" tabIconURL:[weakSelf getFileFullUrl:@"/stickers/local/great.svg"]];
    } category:WKPOINT_CATEGORY_PANELCONTENT sort:3500];
...
//    } category:WKPOINT_CATEGORY_PANELCONTENT sort:1000];
    ...
    // 点赞
    [self setMethod:@"longmenus.like" handler:^id _Nullable(id  _Nonnull param) {
        WKMessageModel *message = param[@"message"];
        if(message.status != WK_MESSAGE_SUCCESS || message.messageId == 0) {
            return nil;
        }
        UIImage *icon = [GenerateImageUtils generateTintedImgWithImage:[weakSelf imageName:@"Conversation/ContextMenu/Favorites"] color:weakSelf.config.contextMenu.primaryColor backgroundColor:nil];
        return [WKMessageLongMenusItem initWithTitle:LLangW(@"点赞", weakSelf) icon:icon onTap:^(id<WKConversationContext> context){
            [[WKSDK shared].reactionManager addOrCancelReaction:@"👍" messageID:message.messageId complete:^(NSError * _Nullable error) {
                if(error) {
                    [[WKNavigationManager shared].topViewController.view showMsg:error.domain ?: LLangW(@"点赞失败", weakSelf)];
                }
            }];
        }];
    } category:WKPOINT_CATEGORY_MESSAGE_LONGMENUS sort:3950];
    ...
    [self setMethod:WKPOINT_LONGMENUS_PIN handler:^id _Nullable(id  _Nonnull param) {
        ...
        BOOL pinned = message.remoteExtra.isPinned || [[WKSDK shared].pinnedMessageManager hasPinned:message.messageId];
        NSString *title = pinned ? LLangW(@"取消置顶", weakSelf) : LLangW(@"置顶", weakSelf);
        UIImage *icon = [GenerateImageUtils generateTintedImgWithImage:[weakSelf imageName:@"Conversation/Messages/Pinned"] color:weakSelf.config.contextMenu.primaryColor backgroundColor:nil];
        return [WKMessageLongMenusItem initWithTitle:title icon:icon onTap:^(id<WKConversationContext> context){
            [[WKAPIClient sharedClient] POST:@"message/pinned" parameters:@{
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:WKNOTIFY_SYNC_PINNED_MESSAGE object:message.channel];
            }).catch(^(NSError *error){
                [[WKNavigationManager shared].topViewController.view showMsg:error.domain ?: LLangW(@"置顶失败", weakSelf)];
            });
        }];
